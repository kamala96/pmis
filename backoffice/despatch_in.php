      ...
      <h3 class="text-themecolor"><i class="fa fa-braille" style="color:#1976d2"></i>&nbsp 
           ...
           <form method="POST" action="index.php?em_id=<?php echo base64_encode($_SESSION['em_id'])?>">
                      ...
                      if ($_SESSION['role'] == 'EMPLOYEE' || $_SESSION['role'] == 'AGENT' || $_SESSION['role'] == 'SUPERVISOR') {
                                      ...
                                      $sql="SELECT * FROM `despatch` WHERE `despatch_status` = 'Sent' AND `region_to` = '$o_region' AND `branch_to` = '$o_branch'  AND date(`datetime`) = '$date' ORDER BY `desp_id` DESC";

                                    }elseif ($_SESSION['role'] == 'RM') {

                                       $sql="SELECT * FROM `despatch` WHERE `despatch_status` = 'Sent' AND `region_to` = '$o_region'  AND date(`datetime`) = '$date' ORDER BY `desp_id` DESC";
