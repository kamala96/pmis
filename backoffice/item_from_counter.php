...
if ($_SESSION['role'] == 'EMPLOYEE' || $_SESSION['role'] == 'AGENT') {
...
$sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`track_number`,`receiver_info`.`r_region`,`transactions`.`office_name`,`bag_status`,`id`
			...
			WHERE `transactions`.`status` = 'Paid' AND `transactions`.`PaymentFor` = 'EMS' AND  `transactions`.`office_name` = 'Back'  AND `transactions`.`bag_status`= 'isNotBag' AND date(`sender_info`.`date_registered`)= '$date' AND `sender_info`.`s_region` = '$o_region' AND `sender_info`.`s_district` = '$o_branch'  ORDER BY `sender_info`.`sender_id` DESC";

}elseif($_SESSION['role'] == 'SUPERVISOR' || $_SESSION['role'] == 'RM'){

	$sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`track_number`,`receiver_info`.`r_region`,`transactions`.`office_name`,`bag_status`,`id`
			FROM `sender_info`
			LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
			LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
			WHERE `transactions`.`status` = 'Paid' AND `transactions`.`PaymentFor` = 'EMS' AND  `transactions`.`office_name` = 'Back'  AND `transactions`.`bag_status`= 'isNotBag' AND date(`sender_info`.`date_registered`)= '$date' AND `sender_info`.`s_region` = '$o_region' ORDER BY `sender_info`.`sender_id` DESC";
}else{
	...
	$sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`track_number`,`receiver_info`.`r_region`,`transactions`.`office_name`,`bag_status`,`id`
			...
			LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
			LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
			WHERE `transactions`.`status` = 'Paid' AND `transactions`.`PaymentFor` = 'EMS' AND  `transactions`.`office_name` = 'Back'  AND `transactions`.`bag_status`= 'isNotBag' AND date(`sender_info`.`date_registered`)= '$date'  ORDER BY `sender_info`.`sender_id` DESC";
}
