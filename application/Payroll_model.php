<?php
  ...
  public function Add_Emp_Percent($data){
    ...
    public function getTotalAdditionAmount($salary_id)
    {

      $month  = date('m');
      $year  = date('Y');
      $sql = "SELECT SUM(add_amount) as add_amount FROM `emp_addition` WHERE `salary_id` = '$salary_id'";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result;
    }
  public function getTotalNonTaxAdditionAmount($salary_id)
    {
      $month  = date('m');
      $year  = date('Y');
      $sql = "SELECT SUM(add_amount) as add_amount FROM `non_tax_addition` WHERE `salary_id` = '$salary_id'";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result;
    }
...
public function getSalaryRecord($id,$year,$monthName){
  ...
  public function Others_Employee_Deduction($salary_id,$id){
    $sql = "SELECT * FROM `others_deduction` WHERE `salary_id` = '$salary_id' ORDER BY `others_id` AND `em_id` = '$id'";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
  }
...
// OR ((MONTH(`end_month`) <= '$month' AND YEAR(`end_month`) > '$year')))
  ...
  public function getAdditional($id,$month,$year){
    $sql = "SELECT * FROM `emp_addition_permonth` WHERE `em_id` = '$id' AND `month` = '$month' AND `year` = '$year' GROUP BY `month`";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
  }
  public function getNonTaxDeduction1($salary_id,$month,$year,$id){
    $sql = "SELECT * FROM `emp_non_percent_deduction_permonth` WHERE `salary_id` = '$salary_id' AND `em_id` = '$id' AND `month` = '$month' AND `year` = '$year' GROUP BY `month`";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
  }
  public function getTaxDeduction1($salary_id,$id,$month,$year){
    $sql = "SELECT * FROM `emp_percent_deduction_permonth` WHERE `salary_id` = '$salary_id' AND `month` = '$month' AND `em_id` = '$id' AND `year` = '$year' GROUP BY `month`";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
  }
