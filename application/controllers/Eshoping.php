...
class Eshoping extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Eshoping_model');
...
public function Eshoping(){
        if ($this->session->userdata('user_login_access') != false) {
            
            $data['cash'] ="";// $this->dashboard_model->get_ems_international();
            $this->session->set_userdata('heading','Eshoping Dashboard');
            $this->load->view('Eshoping/Eshoping-dashboard',@$data);
     ...
     public function Eshoping_List()
    {
        if ($this->session->userdata('user_login_access') != false) {
           ...
           if (!empty($month) || !empty($date) ) {
                $data['list'] = $this->Eshoping_model->get_Eshoping_list_search($date,$month,$region);
               

           } else 
           {

            $data['list'] = $this->Eshoping_model->get_Eshoping_list();

           }


           
           $this->load->view('Eshoping/Eshoping_List',$data);
        } else {
            redirect(base_url());
        }
	...
	public function Eshoping_form()
    {
      ...
      $this->load->view('Eshoping/Eshoping_form',$data);
    ...
    public function Save_Eshoping()
    {
        if ($this->session->userdata('user_login_access') != false) {
           
            $EshopingDetails = $this->input->post('EshopingDetails');
            ...
            $serial    = 'Eshoping'.date("YmdHis").$source->reg_code;
            ...
            $this->Eshoping_model->save_Eshopings($data);
...
$data1 = array();
            ...
            $this->Eshoping_model->save_transactions($data1);
            ...
            $region = $o_region;
            $district = $o_branch;
            $renter   = $EshopingDetails;
            $serviceId = 'ESHOPPING';
            $trackno = '90'.$bagsNo;
            $transaction = $this->getBillGepgBillIdEshoping($serial,$paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno);
            $serial1 = $serial;
            ...
            if ($transaction->controlno != '') {
                ...
                $this->Box_Application_model->save_location($data);
                 ...
                 $data['result'] ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya Eshoping,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                 $total ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya Eshoping,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                $this->Sms_model->send_sms_trick($mobile,$total);

                $this->load->view('Eshoping/Eshoping_control_number',$data);
            }else{
                redirect('Eshoping/Eshoping_List');
            }
...
public function getBillGepgBillIdEshoping($serial, $paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno){
