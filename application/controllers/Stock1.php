	...
	class Stock extends CI_Controller {
		...
		public function Stock_Request(){
			$data['category'] = $this->Stampbureau->CategoryList();
			$this->load->view('Stock/Stock_Request',$data);
		}
		...
		public function Add_Stock()
		{
			if ($this->session->userdata('user_login_access') != false)
			{
				$Id = base64_decode($this->input->get('stockId'));
				$data['stockById'] = $this->Stampbureau->getStockById($Id);
				$data['category'] = $this->Stampbureau->CategoryList();
		...
		public function Counter_Request()
		{
			if ($this->session->userdata('user_login_access') != false)
			{
				...
				$branch=$em_branch;
				 ...
				 $data['stock'] = $this->Stampbureau->getstockcounter($db,$branch);
	...
	public function viewrequest($serialno)
		...
		public function ViewSold()
				{
				  ...
				  $data['controlNo'] = $this->Stampbureau->Selling();
				  $this->load->view('Stock/ViewSold',$data);
				//
           ...
           public function GenerateControlNo()
				{
				  ...
				  $ungenerated = $this->Stampbureau->Sellingdate();

				  $GENERATEDBILL = "098909889053";
                 
				  foreach ($ungenerated as $key => $value) {
				  	# code...

				  	 
				    $GeneratedBillNumber=  $GENERATEDBILL ;
				     $isBillGenerated= "1";
				     $Id = $value->Id;

				  $data = array(
						'isBillGenerated' => $isBillGenerated,
						'GeneratedBillNumber' => $GeneratedBillNumber,
						
					);
				  
		          $db="sellingstocks";
				$success = $this->Stampbureau->Updatedb($db,$data,$Id);
		...
		public function acceptReceived($serialno)
		{
			...
			elseif($formno == "P93")
			{
				$this->load->view('Stock/acceptReceivedP93',$data);
			}
			 elseif($formno == "S64"){
		...
		public function Branch_Request()
		{
			if ($this->session->userdata('user_login_access') != false)
			{
				...
				$region=$em_region;
				 ...
				 $data['stock'] = $this->Stampbureau-> getstockbranch($db,$region);
...
public function Selling()
		{
			if ($this->session->userdata('user_login_access') != false)
			{
				 $db="counterstock";
				 $data['serial'] = $this->Stampbureau->lastserialnumber() ;
				 $data['stock'] = $this->Stampbureau->getstockss($db);
				 $data['philatel'] = $this->Stampbureau->philatelist();
				$this->load->view('Stock/Selling',$data);
			}
			else{
				redirect(base_url());
			}
      ...
      public function UnpaidControlNo()
		{
			if ($this->session->userdata('user_login_access') != false)
			{
				 $data['serial'] = $this->Stampbureau->lastserialnumber() ;
				 $data['stock'] = $this->Stampbureau->getstocklist();
				 $data['controlNo'] = $this->Stampbureau->gencolist();
				  $this->load->view('Stock/UnpaidControlNo',$data);
			}
			else{
				redirect(base_url());
			}
      ...
      public function PaidControlNo()
		{
			if ($this->session->userdata('user_login_access') != false)
			{
				 $data['serial'] = $this->Stampbureau->lastserialnumber() ;
				 $data['stock'] = $this->Stampbureau->getstocklist();
				 $data['controlNo'] = $this->Stampbureau->gencolist1();
				  $this->load->view('Stock/PaidControlNo',$data);
			}
			else{
				redirect(base_url());
			}
...
public function Savephilatel(){ 
	...
	$stocktype = $this->input->post('stock_type');
		 ...
		 $createdby = $em_id ;
			{
             ...
             if (empty($stockId)) {
             	$success = $this->Stampbureau->Addstock($data);
                   
                echo "Successfully Added";
             } else {
             	$success = $this->Stampbureau->Update_stock($data,$stockId);
...
public function Savecash(){ 
 ...
 $StockId= $result + 1;
	...
	public function Savestrongroomrequest(){ 
		...
		$stocktype = $this->input->post('stock_type');
           $stockname = $this->input->post('stock_name');
           $quantity  = $this->input->post('quantity');

           $requested_by = $this->session->userdata('sub_user_type');
           $requested_region = $this->session->userdata('user_region');
           ...
           $this->Stock_model->Request_Save($data);
           echo "Successfully Request Saved";
		...
		if($this->session->userdata('user_login_access') != False) {  
			...
			for ($i = 0; $i<count($this->input->post('QuantitySupp')); $i++){
				...
				$StockId = $this->input->post('StockId['.$i.']');
					...
					$SellingValue= $this->Stampbureau->getstockinfo($StockId);
					$SellingPerValue = $SellingValue->pricepermint;  
					$Totalamount = $SellingPerValue * (int)$QuantitySupp;  
			   ...
			   $this->load->library('form_validation');
				
					   {
						   ...
						   $data = array(
							   ...
							   'Description' => $Description,
							    'SellingPerValue' => $SellingPerValue,
							   'Totalamount' => $Totalamount,
							   
						   );
						  $db="sellingstocks";
					   $success = $this->Stampbureau->Addselling($data,$db);
					   ...
					   //reduce
					   $db="counterstock";
					   $stock= $this->Stampbureau->getlateststock($db,$StockId);
                ...
                $denomination =$stock->denomination;
                $pricepermint =$stock->pricepermint;
                $RequisitionFormId =$stock->requisitionformId;
                $priceperfdcover =$stock->priceperfdcover;
                $totalpricepermint =$stock->totalpricepermint;
                $pricepersouverantsheet  =$stock->pricepersouverantsheet;
                $totalpricepersouverantsheet =$stock->totalpricepersouverantsheet;
                ...
                $quantity = $stock->quantity -$QuantitySupp;
                $emp_id =$stock->Emp_id;
                $Region =$stock->Region;
                $RegionBranch =$stock->Regionbranch;
        ...
        //check kama aredy approved
			...
			if($isaproved == 0)
			{
                ...
                $totalpriceperfdcover =$result->totalpriceperfdcover;
						...
						$success = $this->Stampbureau->Addstocks2($data2,$db);
						echo "Successfully " . $RequisitionFormId;
