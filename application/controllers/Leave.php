...
class Leave extends CI_Controller
{
	...
	public function Get_LeaveDetailss()
	{
		if ($this->session->userdata('user_login_access') != False) {

		$fromdate = date("Y-m-d",strtotime($this->input->post('fromdate')));
		$todate =  date("Y-m-d",strtotime($this->input->post('todate')));
		// $Status = $this->input->post('emp_id');
		$Status = $this->input->post('emid');
		...
		$data['emid'] = $emid;
		$data['Status'] = $Status;
		$data['reports'] = $reports = $this->leave_model->GetEmLEaveReport($emid, $fromdate, $todate, $Status);
		...
		if($check == "Print"){
			...
			if($fromdate=='1970-01-01' || $todate=='1970-01-01'){
			    redirect('Leave/Leave_report');
			}


			if(!empty($reports[0]->start_date)){
			
			 $this->load->library('Pdf');
			 $html= $this->load->view('backend/print_leave_report',$data,TRUE);
			 $this->dompdf->loadHtml($html);
			 $this->dompdf->setPaper('A4','potrait');
			 $this->dompdf->render();
			 ob_end_clean();
			 $this->dompdf->stream('print_leave_report'.date('d-m-Y').'.pdf', array("Attachment"=>FALSE));
			}
	...
	public function Get_LeaveDetails()
	{
		...
		$data['report'] = $report = $this->leave_model->GetEmLEaveReport($emid, $fromdate, $todate, $Status);
	...
	public function print_leave_report()
{
...
$data['emid'] = $emid;
$data['Status'] = $Status;
$reports =  $this->leave_model->GetEmLEaveReport($emid, $fromdate, $todate, $Status);
...
{
$this->session->set_flashdata('feedback','Please! Leave report not found, Please try again..');
redirect($this->agent->referrer());
	...
	public function Application_Form()
	{
	  ...
	  public function Getleavedays(){
	  	  ...
	  	  $em_id = $this->input->post('em_id',TRUE);
	  	  $remainannualdays=0;
            ...
            if (empty($checkDay1)) {
            	 ...
            	 $remainannualdays=$checkDay1->day;
            }  

			$leavedays= $this->leave_model->GetleavetypeInfoid($leaveid);
			$day=1;
			$output ='<option value="'.$day.'">'.$day.'</option>';
			$leavedaysallowed=$leavedays->leave_day;
			// if($leavedays->name =='Study Leave'){
			// 	$leavedaysallowed=28;
			...
			if($leavedays->name =='Annual Leave'){
				if($leavedaysallowed > $remainannualdays){
					while ($remainannualdays > $day) {  //$leavedays->leave_day > $day && 
					// code...
					$day=$day+1;
					$output .='<option value="'.$day.'">'.$day.'</option>';
				     }
				...
				while ($leavedaysallowed > $day) {  //$leavedays->leave_day > $day && 
				// code...
				$day=$day+1;
				$output .='<option value="'.$day.'">'.$day.'</option>';
			    }
	...
	public function Leave_Application()
	{
		   ...
		   $staff = $this->session->userdata('user_login_id');
			...
			if ($this->session->userdata('user_type') == 'HOD' || $this->session->userdata('user_type') == 'ADMIN' ) {
				if ($status == 'Rejected')
				{
					...
					$data1 = array('isHOD' => $status,'leave_status'=>$status,'rejected_reason'=>$rejected_reason,
				                    'HOD_emid' => $staff,'HOD_Date' => $currentDateTime);
				...
				if ($status == 'Rejected')
				{
					...
					$data1 = array('isRM' => $status,'leave_status'=>$status,'rejected_reason'=>$rejected_reason,
				                    'RM_emid' => $staff,'RM_Date' => $currentDateTime);
				}else{
				...
				if ($status == 'Rejected'){
				    ...
				    $this->leave_model->updateDay($id,$update);
						...
						$data1 = array('isHR' => $status,'leave_status'=>$status,'rejected_reason'=>$rejected_reason,'fare_amount' => $amount1,'faredistrictvillage'=>$amount2,
				                    'HR_emid' => $staff,'HR_Date' => $currentDateTime);
				    ...
				    $this->leave_model->updateDay($id,$update);
						...
						$data1 = array('leave_status' => 'Rejected','isPMG' => $status,'rejected_reason'=>$rejected_reason,
				                    'PMG_emid' => $staff,'PMG_Date' => $currentDateTime);
					} else {
				    ...
				    $this->leave_model->updateDay($id,$update);
						...
						$data1 = array('leave_status' => 'Rejected','isGMBOP' => $status,'rejected_reason'=>$rejected_reason,
				                    'GMBOP_emid' => $staff,'GMBOP_Date' => $currentDateTime);
					} else {
			...
			$success = $this->leave_model->Application_Apply_Update1($leaveID, $data1);
				    ...
				    $this->leave_model->updateDay($id,$update);
						...
						$data1 = array('leave_status' => 'Rejected','isGMCRM' => $status,'rejected_reason'=>$rejected_reason,
				                    'GMCRM_emid' => $staff,'GMCRM_Date' => $currentDateTime);
					} else {
			...
			//|| $leavetype == 'Compassionate Leave' || $leavetype == 'Sick Leave'
				if (date('m') >= 07) {
					$year = date('Y')+1;
				} else {
					$year = date('Y');
	...
	public function leaveview(){
        ...
        $emcode = base64_decode($this->input->get('I'));
         $batchNo = base64_decode($this->input->get('NO'));
        $data['basic'] =$basic= $this->employee_model->GetBasic22($emcode);
         $id = $basic->em_id;
        ...
        $data['leavetypes'] = $this->leave_model->GetleavetypeInfo();   
           ...
           $data['report']  =$report = $this->leave_model->GetEmLEaveByemcode($emcode,$batchNo); 
      

        $this->load->view('backend/employee_leave_view',$data);
