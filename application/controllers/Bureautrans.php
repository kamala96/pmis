...
class Bureautrans extends CI_Controller {
...
public function Bureau(){
        if ($this->session->userdata('user_login_access') != false) {
            ...
            $this->session->set_userdata('heading','Bureau Dashboard');
            $this->load->view('Bureau/Bureau-dashboard',@$data);
     ...
     public function Bureau_List()
    {
        if ($this->session->userdata('user_login_access') != false) {
           ...
           if (!empty($month) || !empty($date) ) {
                $data['list'] = $this->Bureau_model->get_Bureau_list_search($date,$month,$region);
               

           } else 
           {

            $data['list'] = $this->Bureau_model->get_Bureau_list();

           }


           
           $this->load->view('Bureau/Bureau_List',$data);
        } else {
            redirect(base_url());
        }
	...
	public function Bureau_form()
    {
      ...
      $this->load->view('Bureau/Bureau_form',$data);
    ...
    public function Save_Bureau()
    {
        if ($this->session->userdata('user_login_access') != false) {
           
            $BureauDetails = $this->input->post('BureauDetails');
            ...
            $serial    = 'Bureau'.date("YmdHis").$source->reg_code;
            ...
            $this->Bureau_model->save_Bureaus($data);
...
$data1 = array();
            ...
            $this->Bureau_model->save_transactions($data1);
            ...
            $serviceId = 'BUREAU';
            $trackno = '90'.$bagsNo;
            $transaction = $this->getBillGepgBillIdBureau($serial,$paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno);
            $serial1 = $serial;
            ...
            if ($transaction->controlno != '') {
                ...
                $this->Box_Application_model->save_location($data);
                 ...
                 $data['result'] ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya Bureau,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                 $total ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya Bureau,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                $this->Sms_model->send_sms_trick($mobile,$total);

                $this->load->view('Bureau/Bureau_control_number',$data);
            }else{
                redirect('Bureau/Bureau_List');
            }
...
public function getBillGepgBillIdBureau($serial, $paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno){
