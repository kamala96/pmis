...
class photocopy extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('photocopy_model');
...
public function photocopy(){
        if ($this->session->userdata('user_login_access') != false) {
            ...
            $this->session->set_userdata('heading','photocopy Dashboard');
            $this->load->view('photocopy/photocopy-dashboard',@$data);
     ...
     public function photocopy_List()
    {
        if ($this->session->userdata('user_login_access') != false) {
           ...
           if (!empty($month) || !empty($date) ) {
                $data['list'] = $this->photocopy_model->get_photocopy_list_search($date,$month,$region);
               

           } else 
           {

            $data['list'] = $this->photocopy_model->get_photocopy_list();

           }


           
           $this->load->view('photocopy/photocopy_List',$data);
        } else {
            redirect(base_url());
        }
	...
	public function photocopy_form()
    {
        if ($this->session->userdata('user_login_access') != false) {
      ...
      $this->load->view('photocopy/photocopy_form',$data);
    ...
    public function Save_photocopy()
    {
        if ($this->session->userdata('user_login_access') != false) {
           
            $photocopyDetails = $this->input->post('photocopyDetails');
            ...
            $serial    = 'photocopy'.date("YmdHis").$source->reg_code;
            ...
            $this->photocopy_model->save_photocopys($data);
...
$data1 = array();
            ...
            'bill_status'=>'PENDING',
            'paymentFor'=>'photocopy'

            );

            $this->photocopy_model->save_transactions($data1);
            ...
            $serviceId = 'PHOTOCOPY';
            $trackno = '90'.$bagsNo;
            $transaction = $this->getBillGepgBillIdphotocopy($serial,$paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno);
            $serial1 = $serial;
            ...
            if ($transaction->controlno != '') {
                ...
                $this->Box_Application_model->save_location($data);
                 ...
                 $data['result'] ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya photocopy,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                 $total ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya photocopy,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                $this->Sms_model->send_sms_trick($mobile,$total);

                $this->load->view('photocopy/photocopy_control_number',$data);
            }else{
                redirect('photocopy/photocopy_List');
            }
...
public function getBillGepgBillIdphotocopy($serial, $paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno){
