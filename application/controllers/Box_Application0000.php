...
$weight = '';
	//$prc = $this->input->post('price');
	$sender = array();
	$sender = array('serial'=>$serial,'ems_type'=>$cargo,'cat_type'=>$emsCat,'weight'=>$weight,'s_fullname'=>$s_fname,'s_address'=>$s_address,'s_email'=>$s_email,'s_mobile'=>$mobile,'s_region'=>$o_region,'s_district'=>$o_branch,'operator'=>$info->em_id);
...
}else{$data['getInfo'] = $this->Box_Application_model->get_despatch_in_per_date($year,$month,$day);}
...
foreach ($fromcounter as $key => $value) {
  ...
  $emslists[] = $this->ReceivedBranch_ViewModel->view_data($value->s_fullname,$value->s_region, $value->s_district, $value->date_registered, $value->track_number,$value->r_region, $value->branch,$value->id, $value->office_name,$value->bag_status, 'COUNTER');
