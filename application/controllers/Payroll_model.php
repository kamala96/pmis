<?php
  ...
  public function Add_Emp_Percent($data){
    ...
    {
      $sql = "SELECT SUM(installment_amount) as others_amount FROM `others_deduction` WHERE `salary_id` = '$salary_id' AND `status` = 'ACTIVE' AND `em_id` = '$id' ORDER BY `others_id` DESC";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result;
    }
    ...
    public function getTotalAdditionAmount($id,$salary_id)
    {
      ...
      $month  = date('m');
      $year  = date('Y');
      $sql = "SELECT SUM(add_amount) as add_amount FROM `emp_addition` WHERE (`em_id` = '$id' AND  `salary_id` = '$salary_id') AND (((`end_month` IS NULL)  OR (MONTH(`end_month`) >= '$month' AND YEAR(`end_month`) = '$year') ) OR (YEAR(`end_month`) > '$year'))
";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result;
  ...
  public function getTotalNonTaxAdditionAmount($id,$salary_id)
    {
      $month  = date('m');
      $year  = date('Y');
      $sql = "SELECT SUM(add_amount) as add_amount FROM `non_tax_addition` WHERE (`em_id` = '$id' AND  `salary_id` = '$salary_id')AND (((`end_month` IS NULL)  OR (MONTH(`end_month`) >= '$month' AND YEAR(`end_month`) = '$year') ) OR (YEAR(`end_month`) > '$year'))
";
...
public function getAssuarance($salary_id)
    ...
    {
      $sql = "SELECT * FROM `assuarance_infor` WHERE `salary_id` = '$salary_id'  AND `em_id` = '$id' ORDER BY  `salary_id` DESC  LIMIT 1";
        $query  = $this->db->query($sql);
        $result = $query->row();
        return $result;
    }
...
public function getSalaryRecord($id,$year,$monthName){
  ...
  public function Others_Employee_Deduction_Permonth($installmentName,$salary_id){
    $sql = "SELECT * FROM `loans_deduction` WHERE `other_names` LIKE '%$installmentName%' AND `salary_id` = '$salary_id' ORDER BY `others_id` DESC LIMIT 1";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result;
  }
  public function getSumNonTaxDeduction($ded_name,$month,$year,$salary_id)
     ...
     public function getSumTaxDeduction($ded_name,$month,$year,$salary_id)
    {
     ...
     public function getSumNonTaxDeductiontotal($ded_name,$id)
    {
  ...
  public function getNonPDeduction($salary_id){
   ...
   public function getTaxAddition($id,$salary_id){
    ...
    $sql = "SELECT * FROM `emp_addition` WHERE (`em_id` LIKE '$id' AND  `salary_id` = '$salary_id')AND (((`end_month` IS NULL)  OR (MONTH(`end_month`) >= '$month' AND YEAR(`end_month`) = '$year') ) OR (YEAR(`end_month`) > '$year'))
";
   ...
   public function getTaxAdditions($id,$salary_id){
    ...
    $sql = "SELECT * FROM `emp_addition` WHERE (`em_id` = '$id' AND  `salary_id` = '$salary_id') AND (((`end_month` IS NULL)  OR (MONTH(`end_month`) >= '$month' AND YEAR(`end_month`) = '$year') ) OR (YEAR(`end_month`) > '$year'))
 ";
  ...
  public function getNonTaxAddition($id,$salary_id){
        $month = date('m');
        $year  = date('Y');
         $sql = "SELECT * FROM `non_tax_addition` WHERE  (`em_id` = '$id' AND  `salary_id` = '$salary_id') AND (((`end_month` IS NULL)  OR (MONTH(`end_month`) >= '$month' AND YEAR(`end_month`) = '$year') ) OR (YEAR(`end_month`) > '$year'))
";
  ...
  public function GetCommulativePension($id){
        $sql = "SELECT MAX(`fund_amount`) AS fund_amount,`fund_name`,`fund_name`,`fund_id`  FROM `pension_fund_contribution`   WHERE  `em_id` = '$id'
  ...
  public function update_heslb($data,$salary_id){
    ...
    public function getAfricanLifesheet($africanlife,$month,$year)
    {
      $sql = "SELECT `assurance_info_months`.`ded_amount` as add_amount,`employee`.*  FROM `assurance_info_months` 
       ...
       WHERE  `assurance_info_months`.`ded_name` = '$africanlife' AND `assurance_info_months`.`month` = '$month' AND `assurance_info_months`.`year` = '$year'";
