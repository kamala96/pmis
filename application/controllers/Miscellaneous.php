...
public function Miscellaneous(){
     ...
     public function Miscellaneous_List()
    {
        if ($this->session->userdata('user_login_access') != false) {
           ...
           if( $this->session->userdata('user_type') != "SUPER ADMIN" && $this->session->userdata('user_type') != "ADMIN" && $this->session->userdata('user_type') != "BOP" && $this->session->userdata('user_type') != "CRM" ){
           ...
           if (!empty($month) || !empty($date) ) {
                $data['list'] = $this->Miscellaneous_model->get_Miscellaneous_list_search($date,$month,$region);
               

           } else 
           {

            $data['list'] = $this->Miscellaneous_model->get_Miscellaneous_list();

           }


           
           $this->load->view('Miscellaneous/Miscellaneous_List',$data);
        } else {
            redirect(base_url());
        }
	...
	public function Miscellaneous_form()
    {
        if ($this->session->userdata('user_login_access') != false) {
      ...
      $this->load->view('Miscellaneous/Miscellaneous_form',$data);
    ...
    public function Save_Miscellaneous()
    {
        if ($this->session->userdata('user_login_access') != false) {
           
            $MiscellaneousDetails = $this->input->post('MiscellaneousDetails');
            ...
            $serial    = 'Miscellaneous'.date("YmdHis").$source->reg_code;
            ...
            $this->Miscellaneous_model->save_Miscellaneouss($data);
...
$data1 = array();
            ...
            'bill_status'=>'PENDING',
            'paymentFor'=>'Miscellaneous'

            );

            $this->Miscellaneous_model->save_transactions($data1);
            ...
            $region = $o_region;
            $district = $o_branch;
            $renter   = $MiscellaneousDetails;
            $serviceId = 'Miscellaneous';
            $trackno = '90'.$bagsNo;
            $transaction = $this->getBillGepgBillIdMiscellaneous($serial,$paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno);
            $serial1 = $serial;
            ...
            if ($transaction->controlno != '') {
                ...
                $this->Box_Application_model->save_location($data);
                 ...
                 $data['result'] ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya Miscellaneous,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                 $total ='KARIBU POSTA KIGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya Miscellaneous,Kiasi unachotakiwa kulipia ni TSH.'.number_format($price,2);

                $this->Sms_model->send_sms_trick($mobile,$total);

                $this->load->view('Miscellaneous/Miscellaneous_control_number',$data);
            }else{
                redirect('Miscellaneous/Miscellaneous_List');
            }
...
public function getBillGepgBillIdMiscellaneous($serial, $paidamount,$region,$district,$mobile,$renter,$serviceId,$trackno){
