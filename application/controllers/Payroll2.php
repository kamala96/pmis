  ...
  public function Save_Othersdecuction()
   ...
   public function Salary_List(){
    ...
    public function Payslip_Create1(){
        ...
        $data['othersDeductions'] = $this->payroll_model->Others_Employee_Deduction($salary_id,$id);
         ...
         foreach ($data['othersDeductions'] as $value) {
            ...
            if(!empty($PensionDeduction)){
                ...
                $NonPercentDeduction= $this->payroll_model->getNonPDeduction($salary_id);
        ...
        $totalNonPDeduction= $this->payroll_model->getTotalNonPDeductionAmount($salary_id);
        $totalDeduction= $this->payroll_model->getTotalDeductionAmount($salary_id);
        $totalPDeduction= $this->payroll_model->getTotalPDeductionAmount($salary_id);
       ...
       if($this->session->userdata('user_login_access') != False) {
        ...
        $bank_name = $this->input->get('bank_name');
             ...
             if (!empty($getSalaryMonth)) {
                 ...
                 echo "<table id='example4' class='table table-bordered' >
                <thead>

                <tr><td>S/N.</td><td>Beneficiary Code</td><td>Beneficiary Name</td><td>Swift Code</td><td>Bank Name</td><td>Account Number</td><td>Amount</td><td>Payment Description</td><td>Title</td></tr>
                </thead><tbody>";
                        $i=0;
                  foreach ($getSalaryMonth as $value) {
                        $i++;

                        echo "<tr><td>".$i."</td><td>".$value->emp_code."</td><td>$value->holder_name</td><td>$value->swift_code</td><td>$value->bank_name</td><td>$value->account_number</td><td>".number_format($value->net_payment,2)."</td><td>$value->payment_desc</td><td>$value->title</td></tr>";
