...
$cust = array();
$cust = array('cust_boxtype'=>$tariffcat,'cust_name'=>$custname,'iddescription'=>$iddescription,'idnumber'=>$idnumber,'first_name'=>$fname,'middle_name'=>$mname,'last_name'=>$lname,'authority_card'=>$authcard);
...
if (empty($box1)) {
	...
	$boxs = array();
	$boxs = array('box_number'=>$box1,'box_status'=>'Occupied','reff_cust_id'=>$last_id);
	$this->Box_Application_model->save_box_number($boxs);
}
...
$box = array();
$box = array('box_status'=>'Occupied','reff_cust_id'=>$cust_id,'region'=>$region,'branch'=>$branch);
...
$serial    = 'EMS'.date("YmdHis").$source->reg_code;
	...
	if ($transaction->controlno != '') {
		...
		$total ='KARIBU POSTA KINGANJANI umepatiwa nambari ya malipo hii hapa'. ' '.$transaction->controlno.' Kwaajili ya huduma ya EMS,Kiasi unachotakiwa kulipia ni TSH.'.number_format($prc,2);

		$total2 ='The amount to be paid for EMS is The TOTAL amount is '.' '.number_format($prc,2).' Pay through this control number'.' '.$transaction->controlno ;
...
$data = array();
$data = array('com_name'=>$comp_name,'com_address'=>$comp_address,'com_phone'=>$comp_phone,'com_region'=>$o_region,'com_branch'=>$o_branch);
												...
												public function Get_Despatch_In_ByMonth()
												{
													...
													public function Credit_Customer_Prepare_Bill(){
														if ($this->session->userdata('user_login_access') != false){
															...
															$data['emslist'] = $this->Box_Application_model->get_credit_customer_list_byAccno($acc_no,$type);
