...
class Demurrage_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_Demurrages($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('Demurrage',$data);
        }
        ...
        public function get_Demurrage_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Demurrage`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Demurrage`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Demurrage`.`region` = '$o_region' AND `Demurrage`.`branch` = '$o_branch' AND `Demurrage`.`Created_byId` = '$id'  ORDER BY `Demurrage`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Demurrage`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Demurrage`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Demurrage`.`region` = '$o_region'    ORDER BY `Demurrage`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Demurrage`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Demurrage`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Demurrage`.`date_created` DESC LIMIT 0";
         ...
         public function get_Demurrage_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Demurrage`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Demurrage`.`region` = '$o_region' AND ( date(`Demurrage`.`date_created`) = '$date')

                   
                          ORDER BY `Demurrage`.`date_created` DESC";

 // AND  ( MONTH(`Demurrage`.`date_created`) = '$day' AND YEAR(`Demurrage`.`Demurrage`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Demurrage`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Demurrage`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Demurrage`.`region` = '$o_region' 
                        AND  ( MONTH(`Demurrage`.`date_created`) = '$day' AND YEAR(`Demurrage`.`date_created`) = '$year' ) 
                          ORDER BY `Demurrage`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Demurrage`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Demurrage`.`region` = '$region' AND ( date(`Demurrage`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`Demurrage`.`date_created`) = '$day' AND YEAR(`Demurrage`.`Demurrage`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Demurrage`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Demurrage`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Demurrage`.`region` = '$region' 
                        AND  ( MONTH(`Demurrage`.`date_created`) = '$day' AND YEAR(`Demurrage`.`date_created`) = '$year' ) 
                          ORDER BY `Demurrage`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Demurrage`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Demurrage`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Demurrage`.`region` = '$o_region' AND `Demurrage`.`Created_byId` = '$id'  
              ORDER BY `Demurrage`.`date_created` DESC";
