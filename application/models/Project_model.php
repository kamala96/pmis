<?php
    ...
    public function GetProjectsValue(){
      ...
      if ($this->session->userdata('user_type')=='EMPLOYEE') {
        $sql = "SELECT * FROM `pro_task` JOIN `assign_task` ON `pro_task`.`id` = `assign_task`.`task_id` JOIN `project` ON `pro_task`.`pro_id` = `project`.`id` WHERE `assign_task`.`assign_user` ='$em_id'";
      }
      else
      {
    ...
    public function GetAllTasksList(){
      ...
      $em_id = $basicinfo->em_id;
      if ($this->session->userdata('user_type')=='EMPLOYEE') {
        $sql = "SELECT `pro_task`.*,`project`.`pro_name`
        FROM `pro_task`
        LEFT JOIN `project` ON `pro_task`.`pro_id`=`project`.`id` 
        LEFT JOIN `assign_task` ON `pro_task`.`id` = `assign_task`.`task_id` 
        WHERE `assign_task`.`assign_user` = '$em_id'";
      }
      else
      {
        $sql = "SELECT `pro_task`.*, `project`.`pro_name`
