...
class Post_shop_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_Post_shops($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('Post_shop',$data);
        }
        ...
        public function get_Post_shop_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Post_shop`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Post_shop`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Post_shop`.`region` = '$o_region' AND `Post_shop`.`branch` = '$o_branch' AND `Post_shop`.`Created_byId` = '$id'  ORDER BY `Post_shop`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM' || $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Post_shop`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Post_shop`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Post_shop`.`region` = '$o_region'    ORDER BY `Post_shop`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Post_shop`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Post_shop`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Post_shop`.`date_created` DESC LIMIT 0";
         ...
         public function get_Post_shop_list_search($date,$month,$region){
            ...
            if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Post_shop`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Post_shop`.`region` = '$o_region' AND ( date(`Post_shop`.`date_created`) = '$date')

                   
                          ORDER BY `Post_shop`.`date_created` DESC";
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Post_shop`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Post_shop`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Post_shop`.`region` = '$o_region' 
                        AND  ( MONTH(`Post_shop`.`date_created`) = '$day' AND YEAR(`Post_shop`.`date_created`) = '$year' ) 
                          ORDER BY `Post_shop`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Post_shop`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Post_shop`.`region` = '$region' AND ( date(`Post_shop`.`date_created`) = '$date')
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Post_shop`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Post_shop`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Post_shop`.`region` = '$region' 
                        AND  ( MONTH(`Post_shop`.`date_created`) = '$day' AND YEAR(`Post_shop`.`date_created`) = '$year' ) 
                          ORDER BY `Post_shop`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Post_shop`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Post_shop`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Post_shop`.`region` = '$o_region' AND `Post_shop`.`Created_byId` = '$id'  
              ORDER BY `Post_shop`.`date_created` DESC";
