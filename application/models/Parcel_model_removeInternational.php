<?php
   ...
   public function update_parcel_international_transactions($update,$serial){
     ...
     public function delete_bulk_international_bysenderid($senderid){
        ...
        $db2->delete('sender_person_info',array('senderp_id'=> $senderid));
         $db2->delete('receiver_register_info',array('sender_id'=> $senderid));
          $db2->delete('parcel_international_transactions',array('register_id'=> $senderid));
      }
    ...
    public function get_code_dest_inter($region_to){
      ...
      $db2 = $this->load->database('otherdb', TRUE);
        $sql = "SELECT * FROM `parcel_international_transactions` WHERE `register_id`='$ids'";
        $query=$db2->query($sql);
        $result = $query->row();
        return $result;
    }
          ...
          public function GetListbulkPacelInternational($operator,$serial){
            ...
            $db2 = $this->load->database('otherdb', TRUE);
                 ...
                 $sql    = "SELECT `sender_person_info`.*,`receiver_register_info`.*,`parcel_international_transactions`.* 
                   ...
                   AND `parcel_international_transactions`.`serial` = '$serial' 
        ...
        $dates =  $date;
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
              ...
              if (!empty($date)) {
                  ...
                  AND  `parcel_international_transactions`.`status` = '$status' ORDER BY `sender_person_info`.`sender_date_created` DESC";
                ...
                if (!empty($date)) {
                  ...
                  WHERE  (date(`sender_person_info`.`sender_date_created`) = '$date')
                  -- AND `sender_person_info`.`sender_type` = 'Parcels-Post' 
                  AND `sender_person_info`.`sender_region` = '$o_region' AND `sender_person_info`.`sender_branch` = '$o_branch' AND  `parcel_international_transactions`.`status` = '$status' ORDER BY `sender_person_info`.`sender_date_created` DESC";
                ...
                if (!empty($date)) {
                  ...
                  WHERE  (date(`sender_person_info`.`sender_date_created`) = '$date')
                  -- AND `sender_person_info`.`sender_type` = 'Parcels-Post' 
                  AND `sender_person_info`.`sender_region` = '$o_region' AND  `parcel_international_transactions`.`status` = '$status' ORDER BY `sender_person_info`.`sender_date_created` DESC";
        ...
        $db2 = $this->load->database('otherdb', TRUE);
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
              ...
              if (!empty($date)) {
                  ...
                  WHERE  (date(`sender_person_info`.`sender_date_created`) = '$date')
                  -- AND `sender_person_info`.`sender_type` = 'Parcels-Post' 
                  AND `sender_person_info`.`sender_region` = '$o_region' AND `sender_person_info`.`sender_branch` = '$o_branch'  AND `sender_person_info`.`operator` = '$emid' AND  `parcel_international_transactions`.`status` = '$status' ORDER BY `sender_person_info`.`sender_date_created` DESC";
