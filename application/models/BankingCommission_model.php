...
class BankingCommission_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_BankingCommissions($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('BankingCommission',$data);
        }
        ...
        public function get_BankingCommission_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `BankingCommission`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`BankingCommission`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `BankingCommission`.`region` = '$o_region' AND `BankingCommission`.`branch` = '$o_branch' AND `BankingCommission`.`Created_byId` = '$id'  ORDER BY `BankingCommission`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `BankingCommission`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`BankingCommission`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `BankingCommission`.`region` = '$o_region'    ORDER BY `BankingCommission`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `BankingCommission`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`BankingCommission`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `BankingCommission`.`date_created` DESC LIMIT 0";
         ...
         public function get_BankingCommission_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `BankingCommission`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `BankingCommission`.`region` = '$o_region' AND ( date(`BankingCommission`.`date_created`) = '$date')

                   
                          ORDER BY `BankingCommission`.`date_created` DESC";

 // AND  ( MONTH(`BankingCommission`.`date_created`) = '$day' AND YEAR(`BankingCommission`.`BankingCommission`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `BankingCommission`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`BankingCommission`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `BankingCommission`.`region` = '$o_region' 
                        AND  ( MONTH(`BankingCommission`.`date_created`) = '$day' AND YEAR(`BankingCommission`.`date_created`) = '$year' ) 
                          ORDER BY `BankingCommission`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `BankingCommission`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `BankingCommission`.`region` = '$region' AND ( date(`BankingCommission`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`BankingCommission`.`date_created`) = '$day' AND YEAR(`BankingCommission`.`BankingCommission`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `BankingCommission`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`BankingCommission`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `BankingCommission`.`region` = '$region' 
                        AND  ( MONTH(`BankingCommission`.`date_created`) = '$day' AND YEAR(`BankingCommission`.`date_created`) = '$year' ) 
                          ORDER BY `BankingCommission`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `BankingCommission`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`BankingCommission`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `BankingCommission`.`region` = '$o_region' AND `BankingCommission`.`Created_byId` = '$id'  
              ORDER BY `BankingCommission`.`date_created` DESC";
