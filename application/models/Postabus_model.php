...
class Postabus_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_Postabus($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('Postabus',$data);
        }
        ...
        public function get_Postabus_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Postabus`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Postabus`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Postabus`.`region` = '$o_region' AND `Postabus`.`branch` = '$o_branch' AND `Postabus`.`Created_byId` = '$id'  ORDER BY `Postabus`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM' || $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Postabus`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Postabus`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Postabus`.`region` = '$o_region'   ORDER BY `Postabus`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Postabus`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Postabus`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Postabus`.`date_created` DESC LIMIT 0";
         ...
         public function get_Postabus_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
              ...
              if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Postabus`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Postabus`.`region` = '$o_region' AND ( date(`Postabus`.`date_created`) = '$date')

                   
                          ORDER BY `Postabus`.`date_created` DESC";
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Postabus`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Postabus`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Postabus`.`region` = '$o_region' 
                        AND  ( MONTH(`Postabus`.`date_created`) = '$day' AND YEAR(`Postabus`.`date_created`) = '$year' ) 
                          ORDER BY `Postabus`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Postabus`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Postabus`.`region` = '$region' AND ( date(`Postabus`.`date_created`) = '$date')
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Postabus`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Postabus`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Postabus`.`region` = '$region' 
                        AND  ( MONTH(`Postabus`.`date_created`) = '$day' AND YEAR(`Postabus`.`date_created`) = '$year' ) 
                          ORDER BY `Postabus`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Postabus`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Postabus`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Postabus`.`region` = '$o_region' AND `Postabus`.`Created_byId` = '$id'  
              ORDER BY `Postabus`.`date_created` DESC";
