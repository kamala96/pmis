...
class Insurance_commision_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_Insurance_commisions($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('Insurance_commision',$data);
        }
        ...
        public function get_Insurance_commision_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Insurance_commision`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Insurance_commision`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Insurance_commision`.`region` = '$o_region' AND `Insurance_commision`.`branch` = '$o_branch' AND `Insurance_commision`.`Created_byId` = '$id'  ORDER BY `Insurance_commision`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Insurance_commision`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Insurance_commision`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Insurance_commision`.`region` = '$o_region'    ORDER BY `Insurance_commision`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Insurance_commision`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Insurance_commision`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Insurance_commision`.`date_created` DESC LIMIT 0";
         ...
         public function get_Insurance_commision_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Insurance_commision`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Insurance_commision`.`region` = '$o_region' AND ( date(`Insurance_commision`.`date_created`) = '$date')

                   
                          ORDER BY `Insurance_commision`.`date_created` DESC";

 // AND  ( MONTH(`Insurance_commision`.`date_created`) = '$day' AND YEAR(`Insurance_commision`.`Insurance_commision`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Insurance_commision`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Insurance_commision`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Insurance_commision`.`region` = '$o_region' 
                        AND  ( MONTH(`Insurance_commision`.`date_created`) = '$day' AND YEAR(`Insurance_commision`.`date_created`) = '$year' ) 
                          ORDER BY `Insurance_commision`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Insurance_commision`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Insurance_commision`.`region` = '$region' AND ( date(`Insurance_commision`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`Insurance_commision`.`date_created`) = '$day' AND YEAR(`Insurance_commision`.`Insurance_commision`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Insurance_commision`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Insurance_commision`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Insurance_commision`.`region` = '$region' 
                        AND  ( MONTH(`Insurance_commision`.`date_created`) = '$day' AND YEAR(`Insurance_commision`.`date_created`) = '$year' ) 
                          ORDER BY `Insurance_commision`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Insurance_commision`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Insurance_commision`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Insurance_commision`.`region` = '$o_region' AND `Insurance_commision`.`Created_byId` = '$id'  
              ORDER BY `Insurance_commision`.`date_created` DESC";
