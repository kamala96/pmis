...
class photocopy_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_photocopys($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('photocopy',$data);
        }
        ...
        public function get_photocopy_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `photocopy`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`photocopy`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `photocopy`.`region` = '$o_region' AND `photocopy`.`branch` = '$o_branch' AND `photocopy`.`Created_byId` = '$id'  ORDER BY `photocopy`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `photocopy`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`photocopy`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `photocopy`.`region` = '$o_region'    ORDER BY `photocopy`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `photocopy`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`photocopy`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `photocopy`.`date_created` DESC LIMIT 0";
         ...
         public function get_photocopy_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `photocopy`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `photocopy`.`region` = '$o_region' AND ( date(`photocopy`.`date_created`) = '$date')

                   
                          ORDER BY `photocopy`.`date_created` DESC";

 // AND  ( MONTH(`photocopy`.`date_created`) = '$day' AND YEAR(`photocopy`.`photocopy`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `photocopy`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`photocopy`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `photocopy`.`region` = '$o_region' 
                        AND  ( MONTH(`photocopy`.`date_created`) = '$day' AND YEAR(`photocopy`.`date_created`) = '$year' ) 
                          ORDER BY `photocopy`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `photocopy`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `photocopy`.`region` = '$region' AND ( date(`photocopy`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`photocopy`.`date_created`) = '$day' AND YEAR(`photocopy`.`photocopy`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `photocopy`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`photocopy`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `photocopy`.`region` = '$region' 
                        AND  ( MONTH(`photocopy`.`date_created`) = '$day' AND YEAR(`photocopy`.`date_created`) = '$year' ) 
                          ORDER BY `photocopy`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `photocopy`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`photocopy`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `photocopy`.`region` = '$o_region' AND `photocopy`.`Created_byId` = '$id'  
              ORDER BY `photocopy`.`date_created` DESC";
