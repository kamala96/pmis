...
class Necta_model extends CI_Model{
        ...
        public function get_bulk_necta_list(){
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
               ...
               $sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`weight`,`track_number`,`rnumber`,`s_district`,`sender_info`.`add_type`,`ems_type`,`receiver_info`.`r_region`,`transactions`.`paidamount`,`paychannel`,`receipt`,`status`,`office_name`,`billid` FROM `sender_info`
            ...
            ORDER BY `sender_info`.`sender_id` DESC";
                ...
                $sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`s_district`,`weight`,`track_number`,`rnumber`,`sender_info`.`add_type`,`ems_type`,`receiver_info`.`r_region`,`transactions`.`paidamount`,`paychannel`,`receipt`,`status`,`office_name`,`billid` FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
              LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `transactions`.`office_name` = 'Counter' AND `transactions`.`paidamount` != 0  ORDER BY `sender_info`.`sender_id` DESC LIMIT 0";
  ...
  public function get_bulk_necta_RegNo_list($serial){
            ...
            $date = date('Y-m-d');
                    ...
                    $sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`weight`,`track_number`,`rnumber`,`sender_info`.`add_type`,`ems_type`,`receiver_info`.`r_region`,`transactions`.`paidamount`,`paychannel`,`receipt`,`status`,`office_name`,`billid` FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
              LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `transactions`.`office_name` = 'Counter' AND `transactions`.`paidamount` != 0 AND `transactions`.`billid` LIKE '$serial' ORDER BY `sender_info`.`sender_id` DESC";
  ...
  public function get_bulk_necta_Groupedlist(){
            ...
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `sender_info`.`s_region` = '$o_region' AND `sender_info`.`s_district` = '$o_branch' AND `transactions`.`office_name` = 'Counter' AND `transactions`.`paidamount` != 0 AND `sender_info`.`operator` = '$id' AND date(`sender_info`.`date_registered`) = '$date' GROUP BY `transactions`.`billid`
            ORDER BY `sender_info`.`sender_id` DESC";

            }elseif($this->session->userdata('user_type') == "RM" || $this->session->userdata('user_type') == "ACCOUNTANT"){

                $sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`weight`,`track_number`,`s_district`,`rnumber`,`sender_info`.`add_type`,`ems_type`,`receiver_info`.`r_region`,`transactions`.`paidamount`,`paychannel`,`receipt`,`status`,`office_name`,`billid` FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
              LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `sender_info`.`s_region` = '$o_region' AND `transactions`.`paidamount` != 0 AND `transactions`.`office_name` = 'Counter' AND date(`sender_info`.`date_registered`) = '$date'  GROUP BY `transactions`.`billid`
            ORDER BY `sender_info`.`sender_id` DESC ";
            ...
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
              LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `sender_info`.`s_region` = '$o_region' AND `sender_info`.`s_district` = '$o_branch' AND `transactions`.`office_name` = 'Counter' AND `transactions`.`paidamount` != 0 AND date(`sender_info`.`date_registered`) = '$date'  GROUP BY `transactions`.`billid` ORDER BY `sender_info`.`sender_id` DESC ";
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `transactions`.`office_name` = 'Counter' AND `transactions`.`paidamount` != 0   GROUP BY `transactions`.`billid` ORDER BY `sender_info`.`sender_id` DESC LIMIT 0";
        ...
        public function get_necta_list(){
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
               ...
               $sql = "SELECT `sender_info`.`s_fullname`,`transactions`.`serial`,`date_registered`,`s_region`,`s_district`,`weight`,`track_number`,`rnumber`,`sender_info`.`add_type`,`ems_type`,`receiver_info`.`r_region`,`ems_tariff_category`.`cat_name`,`transactions`.`paidamount`,`paychannel`,`receipt`,`status`,`office_name`,`billid`,`paychannel` FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
        ...
        public function get_necta_search($date,$month,$status){
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
               ...
               if (!empty($date)) {
                   ...
                   $sql = "SELECT  `sender_info`.`rnumber`,`transactions`.`serial`,`sender_info`.`s_fullname`,`date_registered`,`s_region`,`s_district`,`weight`,`track_number`,`ems_type`,`sender_info`.`add_type`,`receiver_info`.`r_region`,`ems_tariff_category`.`cat_name`,`transactions`.`paidamount`,`status`,`office_name`,`billid`,`paychannel` FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
            LEFT JOIN `ems_tariff_category` ON `ems_tariff_category`.`cat_id`=`sender_info`.`cat_type`
            LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
  ...
  public function get_bulk_necta_search($date,$month,$status){
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
               ...
               if (!empty($date)) {
                   ...
                   $sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`s_district`,`weight`,`track_number`,`ems_type`,`sender_info`.`add_type`,`receiver_info`.`r_region`,`transactions`.`paidamount`,`status`,`office_name`,`paychannel`,`receipt`,`billid`,COUNT(`sender_info`.`serial`) as items FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `sender_info`.`s_region` = '$o_region' AND `sender_info`.`s_district` = '$o_branch' AND `transactions`.`office_name` = 'Counter' AND `sender_info`.`operator` = '$id' AND date(`sender_info`.`date_registered`) = '$date' AND `transactions`.`status` = '$status' AND `transactions`.`paidamount` != 0
           ...
           LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`PaymentFor` = 'NECTA' AND `sender_info`.`s_region` = '$o_region' AND `sender_info`.`s_district` = '$o_branch' AND `transactions`.`office_name` = 'Counter' AND `sender_info`.`operator` = '$id' AND MONTH(`sender_info`.`date_registered`) = '$mon' AND YEAR(`sender_info`.`date_registered`) = '$year' AND `transactions`.`status` = '$status' AND `transactions`.`paidamount` != 0
            GROUP BY `sender_info`.`serial` DESC";
           ...
           LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            ...
            $sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`s_district`,`weight`,`track_number`,`ems_type`,`sender_info`.`add_type`,`receiver_info`.`r_region`,
            `transactions`.`paidamount`,`status`,`office_name`,`paychannel`,`receipt`,`billid`,COUNT(`sender_info`.`serial`) as items
             FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
           LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE  `transactions`.`PaymentFor` = 'NECTA' AND `transactions`.`office_name` = 'Counter' AND date(`sender_info`.`date_registered`) = '$date' AND `transactions`.`status` = '$status' AND `transactions`.`paidamount` != 0
            ...
            $sql = "SELECT `sender_info`.`s_fullname`,`date_registered`,`s_region`,`s_district`,`weight`,`track_number`,
            `ems_type`,`sender_info`.`add_type`,`receiver_info`.`r_region`,`transactions`.`paidamount`,`status`,`office_name`,
            `paychannel`,`receipt`,`billid`,COUNT(`sender_info`.`serial`) as items FROM `sender_info`
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
           LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE `transactions`.`PaymentFor` = 'NECTA' AND `transactions`.`office_name` = 'Counter' AND MONTH(`sender_info`.`date_registered`) = '$mon' AND YEAR(`sender_info`.`date_registered`) = '$year' AND `transactions`.`status` = '$status' AND `transactions`.`paidamount` != 0
            GROUP BY `sender_info`.`serial` ";
        ...
        public function get_bulk_necta_sum(){
            ...
            LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`sender_info`.`serial`
            WHERE `transactions`.`status` = 'Paid' AND `transactions`.`PaymentFor` = 'NECTA'  AND `transactions`.`paidamount` != 0 LIMIT 0"; 
 ...
 public function get_necta_bulk_list($serial){
    $db2 = $this->load->database('otherdb', TRUE);
        $sql = "SELECT  COUNT(`serial`) AS `item`
            FROM `sender_info` WHERE `serial` = '$serial' ";
    ...
    return $result;
            ...
            public function find_all_get_necta_search($fromdata,$todate,$status){
            ...
            LEFT JOIN `ems_tariff_category` ON `ems_tariff_category`.`cat_id`=`sender_info`.`cat_type`
            LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
            WHERE `transactions`.`PaymentFor` = 'NECTA' 
            AND `sender_info`.`operator` = '$id' 
            AND DATE(`sender_info`.`date_registered`) BETWEEN '$fromdata' AND '$todate' 
