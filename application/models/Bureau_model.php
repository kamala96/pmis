...
class Bureau_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_Bureaus($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('Bureau',$data);
        }
        ...
        public function get_Bureau_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Bureau`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Bureau`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Bureau`.`region` = '$o_region' AND `Bureau`.`branch` = '$o_branch' AND `Bureau`.`Created_byId` = '$id'  ORDER BY `Bureau`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Bureau`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Bureau`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Bureau`.`region` = '$o_region'    ORDER BY `Bureau`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Bureau`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Bureau`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Bureau`.`date_created` DESC LIMIT 0";
         ...
         public function get_Bureau_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Bureau`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Bureau`.`region` = '$o_region' AND ( date(`Bureau`.`date_created`) = '$date')

                   
                          ORDER BY `Bureau`.`date_created` DESC";

 // AND  ( MONTH(`Bureau`.`date_created`) = '$day' AND YEAR(`Bureau`.`Bureau`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Bureau`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Bureau`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Bureau`.`region` = '$o_region' 
                        AND  ( MONTH(`Bureau`.`date_created`) = '$day' AND YEAR(`Bureau`.`date_created`) = '$year' ) 
                          ORDER BY `Bureau`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Bureau`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Bureau`.`region` = '$region' AND ( date(`Bureau`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`Bureau`.`date_created`) = '$day' AND YEAR(`Bureau`.`Bureau`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Bureau`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Bureau`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Bureau`.`region` = '$region' 
                        AND  ( MONTH(`Bureau`.`date_created`) = '$day' AND YEAR(`Bureau`.`date_created`) = '$year' ) 
                          ORDER BY `Bureau`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Bureau`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Bureau`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Bureau`.`region` = '$o_region' AND `Bureau`.`Created_byId` = '$id'  
              ORDER BY `Bureau`.`date_created` DESC";
