<?php
    ...
    public function country_name(){
        ...
        $sql = "SELECT * FROM `parcel_international_tarrif` WHERE country !='UNITED STATES'";
    ...
    public function checkValueParcInter($controlno){
      ...
      $db2 = $this->load->database('otherdb', TRUE);
        ...
        AND `weight_step` >= '$weight'  ORDER BY `air_id` LIMIT 1";
        $query  = $db2->query($sql);
        $result = $query->row();
        return $result;
    }

     public function mail_tariff_noweight_price($itemname,$type){
        $db2 = $this->load->database('otherdb', TRUE);
        $sql    = "SELECT * FROM `air_mails_tarrif` WHERE `category`='$itemname'  AND `type`='$type'
         ORDER BY `air_id` LIMIT 1";
        $query  = $db2->query($sql);
        $result = $query->row();
        return $result;
    }

     public function advertising_mail_tariff_price($itemname,$type){
        $db2 = $this->load->database('otherdb', TRUE);
        $sql    = "SELECT * FROM `air_mails_tarrif` WHERE `category`='$itemname'  AND `type`='$type'
                     ORDER BY `air_id` LIMIT 1";
        $query  = $db2->query($sql);
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
         ...
         public function get_advertising_mail_receipt($db,$controlnumber){

            $db2 = $this->load->database('otherdb', TRUE);
            $sql = "SELECT * FROM `air_mails_domestic`
             LEFT JOIN `transactions` ON `transactions`.`serial`=`air_mails_domestic`.`serial`
                    WHERE `air_mails_domestic`.`serial` LIKE '%ADV%'  AND `transactions`.`billid` = '$controlnumber'
         ...
         public function get_air_mails_domestic_advertising_application_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `air_mails_domestic`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `air_mails_domestic`.`region` = '$o_region' AND `air_mails_domestic`.`branch` = '$o_branch' AND `air_mails_domestic`.`Created_byId` = '$id'
            AND `air_mails_domestic`.`serial` LIKE '%ADV%'   ORDER BY `air_mails_domestic`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM' || $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `air_mails_domestic`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`air_mails_domestic`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `air_mails_domestic`.`region` = '$o_region'  
            AND `air_mails_domestic`.`serial` LIKE '%ADV%'   ORDER BY `air_mails_domestic`.`date_created` DESC";
           ...
           $sql = "SELECT * FROM `air_mails_domestic`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `air_mails_domestic`.`serial` LIKE '%ADV%'  LIMIT 0";
    ...
    else
    {
      ...
      $sql = "SELECT * FROM `air_mails_domestic`
        ...
        public function get_air_mails_domestic_advertising_application_list_search($date,$month,$region,$status){
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `air_mails_domestic`.`region` = '$region' 
            AND `air_mails_domestic`.`serial` LIKE '%ADV%' 
             ...
             else
            {
               ...
               $sql = "SELECT * FROM `air_mails_domestic`
                    LEFT JOIN `transactions` ON `transactions`.`serial`=`air_mails_domestic`.`serial`
                    WHERE `transactions`.`status` != 'OldPaid' AND `air_mails_domestic`.`region` = '$o_region' AND `air_mails_domestic`.`Created_byId` = '$id'    AND `air_mails_domestic`.`serial` LIKE '%ADV%' 
                      ORDER BY `air_mails_domestic`.`date_created` DESC";
            ...
            
    {
       ...
       $sql = "SELECT * FROM `air_mails_domestic`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND  `transactions`.`status` = '$status'
            AND `air_mails_domestic`.`region` = '$region'   AND `air_mails_domestic`.`serial` LIKE '%ADV%' 
