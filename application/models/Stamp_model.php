...
class Stamp_model extends CI_Model{
        ...
        public function get_stamp_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Stamp`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `Stamp`.`region` = '$o_region' AND `Stamp`.`branch` = '$o_branch' AND `Stamp`.`Created_byId` = '$id' AND  `Stamp`.`serial`  LIKE '%STAMP%'  ORDER BY `Stamp`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Stamp`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Stamp`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Stamp`.`region` = '$o_region' AND  `Stamp`.`serial`  LIKE '%STAMP%'    ORDER BY `Stamp`.`date_created` DESC";

        }else{
           $sql = "SELECT * FROM `Stamp`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Stamp`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND  `Stamp`.`serial`  LIKE '%STAMP%'   LIMIT 0";
          ...
          public function get_stamp_cash_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
         $sql = "SELECT * FROM `Stamp`
            WHERE `Stamp`.`serial` LIKE '%CASH%' AND  `Stamp`.`region` = '$o_region'
            AND `Stamp`.`branch` = '$o_branch' AND `Stamp`.`Created_byId` = '$id'  ORDER BY `Stamp`.`date_created` DESC";
           ...
           $sql = "SELECT * FROM `Stamp`
            WHERE `Stamp`.`serial` LIKE '%CASH%'   LIMIT 0";
        ...
        public function get_stamp_cash($db,$trackno){

    $db2 = $this->load->database('otherdb', TRUE);
    $sql = "SELECT * FROM `Stamp`
            WHERE `Stamp`.`serial` LIKE '%CASH%'  AND `Stamp`.`serial` = '$trackno'";
  ...
  public function get_stamp_Franking($db,$controlnumber){

    $db2 = $this->load->database('otherdb', TRUE);
    $sql = "SELECT * FROM `Stamp`
     LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`Stamp`.`stamp_id`
            WHERE `Stamp`.`serial` LIKE '%Franking%'  AND `transactions`.`billid` = '$controlnumber'
   ...
   public function get_stamp_cashs($db,$trackno){
    ...
    return $result;
         ...
         public function get_stamp_cash_list_search($date,$month,$region){
        ...
        {
             $sql = "SELECT * FROM `Stamp`
            WHERE `Stamp`.`serial` LIKE '%CASH%' AND  `Stamp`.`region` = '$region'
             AND MONTH(`Stamp`.`date_created`) = '$month3' AND YEAR(`Stamp`.`date_created`) = '$year'
            AND date(`Stamp`.`date_created`) = '$dates'
              ORDER BY `Stamp`.`date_created` DESC";
        ...
        else
        {
         ...
         public function get_stamp_list_search($date,$month,$region){
        ...
        if(!empty($date) && !empty($month) && !empty($region)  )
        {
             $sql = "SELECT * FROM `Stamp`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Stamp`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`region` = '$region'
             AND MONTH(`transactions`.`transactiondate`) = '$month3' AND YEAR(`transactions`.`transactiondate`) = '$year'
            AND date(`transactions`.`transactiondate`) = '$dates' AND  `Stamp`.`serial`  LIKE '%STAMP%'
              ORDER BY `Stamp`.`date_created` DESC";
        ...
        else
        {
            ...
            $sql = "SELECT * FROM `Stamp`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Stamp`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`region` = '$region' AND  `Stamp`.`serial`  LIKE '%STAMP%'
         ...
         public function get_franking_stamp_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Stamp`
           LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`Stamp`.`stamp_id`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `Stamp`.`region` = '$o_region' AND `Stamp`.`branch` = '$o_branch' AND `Stamp`.`Created_byId` = '$id' AND  `Stamp`.`serial` LIKE '%Franking%'  ORDER BY `Stamp`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Stamp`
           LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`Stamp`.`stamp_id`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Stamp`.`region` = '$o_region' 
            AND  `Stamp`.`serial` LIKE '%Franking%'   ORDER BY `Stamp`.`date_created` DESC";

        }else{
           $sql = "SELECT * FROM `Stamp`
           LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`Stamp`.`stamp_id`
            
            WHERE `transactions`.`status` != 'OldPaid' AND  `Stamp`.`serial` LIKE '%Franking%'   LIMIT 0";
         ...
         public function get_franking_stamp_list_search($date,$month,$region){
        ...
        if(!empty($date) && !empty($month) && !empty($region)  )
        {
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`region` = '$region'
             AND MONTH(`transactions`.`transactiondate`) = '$month3' AND YEAR(`transactions`.`transactiondate`) = '$year'
            AND date(`transactions`.`transactiondate`) = '$dates' AND  `Stamp`.`serial` LIKE '%Franking%'
              ORDER BY `Stamp`.`date_created` DESC";
        ...
        elseif( !empty($month) && !empty($region)  )
        {
           ...
           LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`Stamp`.`stamp_id`
            WHERE `transactions`.`status` != 'OldPaid' AND `transactions`.`region` = '$region' 
             AND MONTH(`transactions`.`transactiondate`) = '$month3' AND YEAR(`transactions`.`transactiondate`) = '$year'
          AND  `Stamp`.`serial` LIKE '%Franking%'
              ORDER BY `Stamp`.`date_created` DESC";
