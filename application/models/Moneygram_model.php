...
class Moneygram_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_Moneygrams($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('Moneygram',$data);
        }
        ...
        public function get_Moneygram_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Moneygram`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Moneygram`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Moneygram`.`region` = '$o_region' AND `Moneygram`.`branch` = '$o_branch' AND `Moneygram`.`Created_byId` = '$id'  ORDER BY `Moneygram`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Moneygram`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Moneygram`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Moneygram`.`region` = '$o_region'    ORDER BY `Moneygram`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Moneygram`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Moneygram`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Moneygram`.`date_created` DESC LIMIT 0";
         ...
         public function get_Moneygram_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Moneygram`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Moneygram`.`region` = '$o_region' AND ( date(`Moneygram`.`date_created`) = '$date')

                   
                          ORDER BY `Moneygram`.`date_created` DESC";

 // AND  ( MONTH(`Moneygram`.`date_created`) = '$day' AND YEAR(`Moneygram`.`Moneygram`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Moneygram`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Moneygram`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Moneygram`.`region` = '$o_region' 
                        AND  ( MONTH(`Moneygram`.`date_created`) = '$day' AND YEAR(`Moneygram`.`date_created`) = '$year' ) 
                          ORDER BY `Moneygram`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Moneygram`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Moneygram`.`region` = '$region' AND ( date(`Moneygram`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`Moneygram`.`date_created`) = '$day' AND YEAR(`Moneygram`.`Moneygram`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Moneygram`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Moneygram`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Moneygram`.`region` = '$region' 
                        AND  ( MONTH(`Moneygram`.`date_created`) = '$day' AND YEAR(`Moneygram`.`date_created`) = '$year' ) 
                          ORDER BY `Moneygram`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Moneygram`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Moneygram`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Moneygram`.`region` = '$o_region' AND `Moneygram`.`Created_byId` = '$id'  
              ORDER BY `Moneygram`.`date_created` DESC";
