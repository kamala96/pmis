  ...
  public function get_posts_cargo_application_listed(){
           ...
           /* if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
                ...
                $sql    = "SELECT `sender_info`.*,`receiver_info`.*,`transactions`.* 
                   ...
                   WHERE `sender_info`.`s_region` = '$o_region' AND `sender_info`.`s_district` = '$o_branch'   ORDER BY `transactions`.`id` DESC";
                ...
                $sql    = "SELECT `sender_info`.*,`receiver_info`.*,`transactions`.*
                   ...
                   WHERE `sender_info`.`s_region` = '$o_region' ORDER BY `transactions`.`id` DESC";
                ...
                $sql    = "SELECT `sender_info`.*,`receiver_info`.*,`transactions`.* 
                   ...
                   INNER JOIN  `receiver_info` ON  `receiver_info`.`from_id` = `sender_info`.`sender_id`
                   INNER JOIN  `transactions`  ON   `sender_info`.`serial`   = `transactions`.`serial` 
                    WHERE `sender_info`.`s_region` = '$o_region'
         ...
         $db2 = $this->load->database('otherdb', TRUE);
             ...
             $sql    = "SELECT `sender_info`.*,`receiver_info`.*,`transactions`.*,`posts_cargo`.*
                   FROM   `sender_info`  
                   INNER JOIN  `receiver_info` ON  `receiver_info`.`from_id` = `sender_info`.`sender_id`
                   INNER JOIN  `transactions`  ON   `sender_info`.`serial`   = `transactions`.`serial` 
                   INNER JOIN  `posts_cargo`  ON   `transactions`.`serial`   = `posts_cargo`.`serial`
                    WHERE `transactions`.`status` != 'OldPaid' GROUP BY `transactions`.`serial` 
                     ORDER BY `transactions`.`id` DESC";
        ...
        $db2 = $this->load->database('otherdb', TRUE);
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT") {
              ...
              if (!empty($date)) {
                  ...
                  INNER JOIN  `register_transactions`  ON   `sender_person_info`.`senderp_id`   = `register_transactions`.`register_id`  
                  WHERE  (date(`sender_person_info`.`sender_date_created`) = '$date')
                  AND `sender_person_info`.`sender_type` = 'Parcels-Post' AND `sender_person_info`.`sender_region` = '$o_region' AND `sender_person_info`.`sender_branch` = '$o_branch'  AND `sender_person_info`.`operator` = '$emid' AND `register_transactions`.`status` = 'Paid' ORDER BY `sender_person_info`.`sender_date_created` DESC";
                  ...
                  INNER JOIN  `register_transactions`  ON   `sender_person_info`.`senderp_id`   = `register_transactions`.`register_id`  
                  WHERE  (MONTH(`sender_person_info`.`sender_date_created`) = '$day' AND YEAR(`sender_person_info`.`sender_date_created`) = '$year') AND `sender_person_info`.`sender_region` = '$o_region' AND `sender_person_info`.`sender_branch` = '$o_branch'  AND `sender_person_info`.`operator` = '$emid'
                  AND `sender_person_info`.`sender_type` = 'Parcels-Post' AND `register_transactions`.`status` = 'Paid' ORDER BY `sender_person_info`.`sender_date_created` DESC";
                ...
                if (!empty($date)) {
                  ...
                  INNER JOIN  `register_transactions`  ON   `sender_person_info`.`senderp_id`   = `register_transactions`.`register_id`  
                  WHERE  (date(`sender_person_info`.`sender_date_created`) = '$date')
                  AND `sender_person_info`.`sender_type` = 'Parcels-Post' AND `sender_person_info`.`sender_region` = '$o_region' AND `sender_person_info`.`sender_branch` = '$o_branch' AND `register_transactions`.`status` = 'Paid' ORDER BY `sender_person_info`.`sender_date_created` DESC";
                ...
                if (!empty($date)) {
                  ...
                  INNER JOIN  `register_transactions`  ON   `sender_person_info`.`senderp_id`   = `register_transactions`.`register_id`  
                  WHERE  (date(`sender_person_info`.`sender_date_created`) = '$date')
                  AND `sender_person_info`.`sender_type` = 'Parcels-Post' AND `sender_person_info`.`sender_region` = '$o_region' AND `register_transactions`.`status` = 'Paid' ORDER BY `sender_person_info`.`sender_date_created` DESC";
    ...
    public function search_register_application_list($date,$month,$status,$branch){
    ...
    public function get_register_sum_search($date,$month,$status,$branch){
        ...
        $db2 = $this->load->database('otherdb', TRUE);
            ...
            if ($this->session->userdata('user_type') == "EMPLOYEE" || $this->session->userdata('user_type') == "AGENT" || $this->session->userdata('user_type') == "SUPERVISOR") {
                ...
                $sql    = "SELECT `sender_person_info`.*,`receiver_register_info`.`add_type` as receivertype,`receiver_register_info`.*,`register_transactions`.* 
                  ...
                  OR `sender_person_info`.`sender_status` = 'Derivery') AND `sender_person_info`.`sender_type` = '$service_type'  ORDER BY  `sender_person_info`.`sender_date_created` DESC";
                ...
                $sql    = "SELECT `sender_person_info`.*,`receiver_register_info`.`add_type` as receivertype,`receiver_register_info`.*,`register_transactions`.* 
                   ...
                   INNER JOIN  `receiver_register_info` ON  `receiver_register_info`.`sender_id` = `sender_person_info`.`senderp_id`
                   INNER JOIN  `register_transactions`  ON   `sender_person_info`.`senderp_id`   = `register_transactions`.`register_id`  
                   WHERE `receiver_register_info`.`receiver_region` = '$o_region' AND `receiver_register_info`.`sender_type` = '$service_type' AND ( `sender_person_info`.`sender_status` = 'Received' OR `sender_person_info`.`sender_status` = 'Derivery') ORDER BY `sender_person_info`.`sender_date_created` DESC";
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `registered_international`
           ...
           $sql = "SELECT * FROM `registered_international`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `registered_international`.`region` = '$o_region' AND   ORDER BY `registered_international`.`date_created` DESC";

        }else{
           $sql = "SELECT * FROM `registered_international`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`registered_international`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `registered_international`.`date_created` DESC";
