    ...
    //   }
      ...
      public function check_region($id) {
      	...
      	$db2 = $this->load->database('otherdb', TRUE);
				 $sql = "SELECT *
		          FROM `zone_employee`
		           WHERE `emid` = '$id'  ORDER BY `id` DESC LIMIT 1";
				$result=$db2->query($sql);
				if ($result->row()) {
					return $result->row();
				} else {
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
	...
	public  function get_ems_bags_list_in_per_date($bagdate){
      ...
      if ( $this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR')
        {
	    $sql = "SELECT * FROM bags WHERE DATE(date_created) = '$bagdate' AND bag_region_from='$o_region' AND bag_branch_from='$o_branch'";
        }
        else
        {
        $sql = "SELECT * FROM bags WHERE DATE(date_created) = '$bagdate'";
        }
...
public  function get_box_payment_paid_list_perperson($id){
	...
	public  function get_box_outstanding_list_perperson($id){
		...
		WHERE `transactions`.`PaymentFor` = 'POSTSBOX' AND `transactions`.`CustomerID` = '$id'  ORDER BY `Outstanding`.`year` ASC";
...
AND YEAR(`sender_info`.`date_registered`) = '$year' AND `transactions`.`status` = 'Bill'";
	...
	public  function get_item_received_from_branch_list(){
		...
		if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
			...
			AND `bags`.`bags_status` = 'isReceived' AND `sender_info`.`s_region` = '$o_region'  AND  `transactions`.`PaymentFor` = 'EMS' 
			ORDER BY `sender_info`.`sender_id` DESC ";
			...
			AND `bags`.`bags_status` = 'isReceived' AND `sender_info`.`s_region` = '$o_region'  
			 AND  `transactions`.`PaymentFor` = 'EMS'
			ORDER BY `sender_info`.`sender_id` DESC ";
			...
			LEFT JOIN `bags` ON `bags`.`bag_number`=`transactions`.`isBagNo`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') 
			AND `bags`.`bags_status` = 'isReceived' AND  `transactions`.`PaymentFor` = 'EMS'
			ORDER BY `sender_info`.`sender_id` DESC";
...
public  function get_item_received_from_outside_Region_hpo_list(){
		...
		$id = $this->session->userdata('user_login_id');
            ...
            AND `bags`.`bags_status` = 'isReceived' AND `receiver_info`.`r_region` = '$o_region' 
              AND  `transactions`.`PaymentFor` = 'EMS'  AND `sender_info`.`item_status`!='Derivered'
...
public  function get_item_received_from_outside_Region_list(){
		...
		if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
			...
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') 
			AND `bags`.`bags_status` = 'isReceived' AND `receiver_info`.`r_region` = '$o_region' 
			 AND `receiver_info`.`branch` = '$o_branch' AND  `transactions`.`PaymentFor` = 'EMS' 
...
public  function get_item_received_list_search($date,$month,$region){
	...
	$m = explode('-', $month);
		...
		if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
			...
			LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
			LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') AND `transactions`.`office_name` = 'Received' AND `sender_info`.`s_region` = '$o_region' AND `transactions`.`bag_status` = 'isNotBag' AND `sender_info`.`s_district` = '$o_branch' AND  `transactions`.`PaymentFor` = 'EMS' AND date(`sender_info`.`date_registered`) = '$date'
			ORDER BY `sender_info`.`sender_id` DESC ";
			...
			LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
			LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') AND `transactions`.`office_name` = 'Received' AND `sender_info`.`s_region` = '$o_region' AND `transactions`.`bag_status` = 'isNotBag' AND  `transactions`.`PaymentFor` = 'EMS'AND date(`sender_info`.`date_registered`) = '$date'
			ORDER BY `sender_info`.`sender_id` DESC ";
			...
			LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
			LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') AND `transactions`.`office_name` = 'Received' AND `sender_info`.`s_region` = '$o_region' AND `transactions`.`bag_status` = 'isNotBag' AND  `transactions`.`PaymentFor` = 'EMS'AND date(`sender_info`.`date_registered`) = '$today'
			ORDER BY `sender_info`.`sender_id` DESC ";
			...
			AND `sender_info`.`s_region` = '$region'
			 AND  `transactions`.`PaymentFor` = 'EMS' AND date(`sender_info`.`date_registered`) = '$date'  ORDER BY `sender_info`.`sender_id` DESC";
			...
			LEFT JOIN `receiver_info` ON `receiver_info`.`from_id`=`sender_info`.`sender_id`
			LEFT JOIN `transactions` ON `transactions`.`CustomerID`=`sender_info`.`sender_id`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') AND `transactions`.`bag_status` = 'isNotBag' AND `transactions`.`office_name` = 'Received' AND  `transactions`.`PaymentFor` = 'EMS'
			AND `sender_info`.`s_region` = '$region'
	...
	$m = explode('-', $month);
		...
		$id = $this->session->userdata('user_login_id');
			...
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') 
			AND `bags`.`bags_status` = 'isReceived' AND `receiver_info`.`r_region` = '$o_region' 
			 AND  `transactions`.`PaymentFor` = 'EMS' AND (`zone_employee`.`emid` = '$id' )
			  AND `sender_info`.`item_status`!='Derivered' AND `sender_info`.`item_status`!='Assigned'
			 AND `sender_info`.`item_status`!='WaitingDelivery'
			AND MONTH(`sender_info`.`date_registered`) = '$day' AND YEAR(`sender_info`.`date_registered`) = '$year' 
			ORDER BY `sender_info`.`sender_id` DESC ";
	...
	$m = explode('-', $month);
		...
		if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
			...
			LEFT JOIN `bags` ON `bags`.`bag_number`=`transactions`.`isBagNo`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') 
			AND `bags`.`bags_status` = 'isReceived' AND `receiver_info`.`r_region` = '$o_region'  AND `receiver_info`.`branch` = '$o_branch' AND  `transactions`.`PaymentFor` = 'EMS'  AND `sender_info`.`item_status`!='Derivered'
			AND `sender_info`.`item_status`!='Assigned'
			 AND `sender_info`.`item_status`!='WaitingDelivery'
			 AND date(`sender_info`.`date_registered`) = '$date'
			ORDER BY `sender_info`.`sender_id` DESC ";
	...
	$m = explode('-', $month);
		...
		if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
			...
			LEFT JOIN `bags` ON `bags`.`bag_number`=`transactions`.`isBagNo`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') 
			AND `bags`.`bags_status` = 'isReceived' AND `receiver_info`.`r_region` = '$o_region'  AND `receiver_info`.`branch` = '$o_branch' AND  `transactions`.`PaymentFor` = 'EMS' AND date(`sender_info`.`date_registered`) = '$date'
			ORDER BY `sender_info`.`sender_id` DESC ";
			...
			LEFT JOIN `bags` ON `bags`.`bag_number`=`transactions`.`isBagNo`
			WHERE (`transactions`.`status` = 'Paid' OR `transactions`.`status` = 'Bill') 
			AND `bags`.`bags_status` = 'isReceived' AND `receiver_info`.`r_region` = '$o_region'  AND `receiver_info`.`branch` = '$o_branch' AND  `transactions`.`PaymentFor` = 'EMS'
			AND MONTH(`sender_info`.`date_registered`) = '$day' AND YEAR(`sender_info`.`date_registered`) = '$year' 
			ORDER BY `sender_info`.`sender_id` DESC ";
...
public  function get_ems_back_list_Search($date,$month){
	...
	public  function get_ems_bags_list(){
		...
		if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
			$sql = "SELECT * FROM `bags` WHERE `bags`.`bag_region_from` = '$o_region' AND `bags`.`bag_branch_from` = '$o_branch' AND `bags`.`bags_status` = 'notDespatch' AND date(`bags`.`date_created`) > '$STARTdate' AND `ems_category` = 'Domestic' ORDER BY `bags`.`date_created` DESC";
		}else{
			$sql = "SELECT * FROM `bags` WHERE  `bags`.`bags_status` = 'notDespatch' AND date(`bags`.`date_created`) > '$STARTdate' AND `ems_category` = 'Domestic' ORDER BY `bags`.`bag_id` DESC";
		}
...
public function get_ems_sum12($year,$month,$day){
    ...
    public function take_bags_desp_listtwo($type,$despno){
		...
		$db2 = $this->load->database('otherdb', TRUE);
		//$sql = "SELECT * FROM `bags` WHERE `despatch_no`='$despno' && `service_type` = '$type'";
		$sql="SELECT * FROM bags INNER JOIN despatch ON despatch.desp_no = bags.despatch_no where bags.despatch_no='$despno'";
		$query  = $db2->query($sql);
		$result = $query->row();
		return $result;
	}
	...
	public function count_bags(){
		...
		if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
			...
			$sql = "SELECT * FROM `bags` WHERE `bags`.`bag_region_from` = '$o_region' AND `bags`.`bag_branch_from` = '$o_branch' AND `bags`.`bags_status` = 'notDespatch' AND date(`bags`.`date_created`) > '$STARTdate' ORDER BY `bags`.`date_created` DESC";
		}else{
			$sql = "SELECT * FROM `bags` WHERE `bags`.`bags_status` = 'notDespatch' AND date(`bags`.`date_created`) > '$STARTdate' ORDER BY `bags`.`date_created` DESC";
		}
