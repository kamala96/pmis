...
class Eshoping_model extends CI_Model{
        ...
        public function save_transactions($data1){
         ...
         public function save_Eshopings($data){
            $db2 = $this->load->database('otherdb', TRUE);
            $db2->insert('Eshoping',$data);
        }
        ...
        public function get_Eshoping_list(){
...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Eshoping`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Eshoping`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Eshoping`.`region` = '$o_region' AND `Eshoping`.`branch` = '$o_branch' AND `Eshoping`.`Created_byId` = '$id'  ORDER BY `Eshoping`.`date_created` DESC";

        }elseif($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
           $sql = "SELECT * FROM `Eshoping`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Eshoping`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid' AND `Eshoping`.`region` = '$o_region'    ORDER BY `Eshoping`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Eshoping`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Eshoping`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Eshoping`.`date_created` DESC LIMIT 0";
         ...
         public function get_Eshoping_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Eshoping`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Eshoping`.`region` = '$o_region' AND ( date(`Eshoping`.`date_created`) = '$date')

                   
                          ORDER BY `Eshoping`.`date_created` DESC";

 // AND  ( MONTH(`Eshoping`.`date_created`) = '$day' AND YEAR(`Eshoping`.`Eshoping`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Eshoping`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Eshoping`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Eshoping`.`region` = '$o_region' 
                        AND  ( MONTH(`Eshoping`.`date_created`) = '$day' AND YEAR(`Eshoping`.`date_created`) = '$year' ) 
                          ORDER BY `Eshoping`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Eshoping`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Eshoping`.`region` = '$region' AND ( date(`Eshoping`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`Eshoping`.`date_created`) = '$day' AND YEAR(`Eshoping`.`Eshoping`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Eshoping`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Eshoping`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Eshoping`.`region` = '$region' 
                        AND  ( MONTH(`Eshoping`.`date_created`) = '$day' AND YEAR(`Eshoping`.`date_created`) = '$year' ) 
                          ORDER BY `Eshoping`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Eshoping`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Eshoping`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Eshoping`.`region` = '$o_region' AND `Eshoping`.`Created_byId` = '$id'  
              ORDER BY `Eshoping`.`date_created` DESC";
