...
if ($this->session->userdata('user_type') == 'EMPLOYEE' || $this->session->userdata('user_type') == 'AGENT' || $this->session->userdata('user_type') == 'SUPERVISOR') {
          $sql = "SELECT * FROM `Internet`
           ...
           $sql = "SELECT * FROM `Internet`
            ...
            WHERE `transactions`.`status` != 'OldPaid' AND `Internet`.`region` = '$o_region'    ORDER BY `Internet`.`date_created` DESC LIMIT 0";

        }else{
           $sql = "SELECT * FROM `Internet`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Internet`.`serial`
            
            WHERE `transactions`.`status` != 'OldPaid'  ORDER BY `Internet`.`date_created` DESC LIMIT 0";
         ...
         public function get_Internet_list_search($date,$month,$region){
             ...
             $id = $info->em_code;
                ...
                if($this->session->userdata('user_type') == 'RM'|| $this->session->userdata('user_type') == "ACCOUNTANT"){
                     ...
                     $sql = "SELECT * FROM `Internet`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Internet`.`region` = '$o_region' AND ( date(`Internet`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`Internet`.`date_created`) = '$day' AND YEAR(`Internet`.`Internet`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Internet`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Internet`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Internet`.`region` = '$o_region' 
                        AND  ( MONTH(`Internet`.`date_created`) = '$day' AND YEAR(`Internet`.`date_created`) = '$year' ) 
                          ORDER BY `Internet`.`date_created` DESC";
              ...
              if ((!empty($month) || !empty($date)) && !empty($region)) {
                   ...
                   if (!empty($date)) {
                     $sql = "SELECT * FROM `Internet`
                        ...
                        WHERE `transactions`.`status` != 'OldPaid' AND `Internet`.`region` = '$region' AND ( date(`Internet`.`date_created`) = '$date')
 ...
 // AND  ( MONTH(`Internet`.`date_created`) = '$day' AND YEAR(`Internet`.`Internet`) = '$year' ) 
                        ...
                        else
                        {
                             $sql = "SELECT * FROM `Internet`
                        LEFT JOIN `transactions` ON `transactions`.`serial`=`Internet`.`serial`
                        WHERE `transactions`.`status` != 'OldPaid' AND `Internet`.`region` = '$region' 
                        AND  ( MONTH(`Internet`.`date_created`) = '$day' AND YEAR(`Internet`.`date_created`) = '$year' ) 
                          ORDER BY `Internet`.`date_created` DESC";
            ...
            else
            {
                 $sql = "SELECT * FROM `Internet`
            LEFT JOIN `transactions` ON `transactions`.`serial`=`Internet`.`serial`
            WHERE `transactions`.`status` != 'OldPaid' AND `Internet`.`region` = '$o_region' AND `Internet`.`Created_byId` = '$id'  
              ORDER BY `Internet`.`date_created` DESC";
