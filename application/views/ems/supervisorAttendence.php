...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        ...
        <h3 class="text-themecolor"><i class="fa fa-clone" style="color:#1976d2"> </i> supervisor Attendence </h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                <li class="breadcrumb-item active">supervisor Attendence</li>
            </ol>
        </div>
    </div>
    <!-- Container fluid  -->
                ...
                <div class="card card-outline-info">
                    <div class="card-body">
                        ...
                        <table id="example4" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody class="results">
                                           <?php foreach($superAtt as $value): ?>
                                            <tr>
                                                <td><?php $id = $value->supervisor_name;
													@$info = $this->employee_model->getBasic($id);
													echo  'PF:'.' '. @$info->em_code. '  ' . @$info->first_name. '  ' .@$info->middle_name. '  ' .@$info->last_name; 
												?></td>
                                                <td><?php echo $value->endday_date;?></td>
                                                <td><?php 
												@$id = @$value->supervisee_name;
													@$info = $this->employee_model->getBasic($id);
													echo  'PF:'.' '. @$info->em_code. '  ' . @$info->first_name. '  ' .@$info->middle_name. '  ' .@$info->last_name;
												?></td>
												<td>
												<?php 
											  if($value->sup_status == "ShiftEnd"){
												  echo "<button class='btn btn-info btn-sm'>Waiting To End Shift</button>";
											  }elseif($value->sup_status == "NotEnded"){
												  	  echo "<button class='btn btn-warning btn-sm'>Shift Not End</button>";
											  }else{
												  	  echo "<button class='btn btn-success btn-sm'>Shift Ended</button>";
											  }?>
												</td>
												<td> <a href="<?php echo base_url(); ?>Box_Application/Ems_Application_List?I=<?php echo base64_encode(@$value->supervisee_name); ?>" title="Edit" class="btn  btn-success btn-sm waves-effect waves-light">Track Job</a></td>
                                            </tr>
                                            <?php endforeach; ?>
