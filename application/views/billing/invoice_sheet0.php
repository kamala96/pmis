...
<body style="text-align: center;font-family: monospace; background-size: 100%;">
  ...
  <table style="width: 100%;font-size: 14px;border-bottom: solid; 1px;border-top: solid;1px;">
      ...
      <?php foreach ($emslist as $value) {?>
         <tr>
           ...
           <td style="text-align: right;">
             ...
             <?php if(!empty($value->paidamount)){
        echo number_format(($value->paidamount* 100)/118,2);
      }else{
        echo number_format(($value->amount* 100)/118,2);
      } ?>
           </td>
         </tr>
     <?php  } ?>
     ...
     <tfoot>
      ...
      <tr><td colspan="2"></td></tr>
      <tr><td>VAT[VAT RATE = 18%]</td><td>Tsh.</td><td style="text-align: right;"><b><?php if(!empty($sum->paidamount)){
        echo number_format(@$sum->paidamount*0.18,2);
      }else{
        echo number_format(@$sum->total*0.18,2);
      } ?></b></td></tr>
       <tr><td>TOTAL::</td><td>Tsh.</td><td style="text-align: right;"><b><?php if(!empty($sum->paidamount)){
        echo number_format(@$sum->paidamount,2);
      }else{
