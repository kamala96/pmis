            ...
            <div class="col-12">
                <div class="card card-outline-info">
                    <div class="card-header">
                        <h4 class="m-b-0 text-white">Imprest Expenditure List
                        </h4>
                    </div>
						...
						<thead>
							...
							<tbody>
							  ...
							  <tr>
								  ...
								  <td><?php echo $value->exp_type; ?></td>
								  <td><?php echo $value->date_created; ?></td>
								  <td>
								  	<?php if($value->exp_status == 'isNON'){?>
								  		<button class="btn btn-primary btn-sm" class="button" disabled>Waiting For ACC Approve.</button>
								  	<?php }elseif($value->exp_status == 'isACC'){?>
								  		<button class="btn btn-info btn-sm" class="button" disabled>Waiting For GMCRM/GMBOP Approve.</button>
								  	<?php }elseif($value->exp_status == 'isGMCRM' || $value->exp_status == 'isGMBOP'){?>
								  		<button class="btn btn-info btn-sm" class="button" disabled>Waiting For PMG Approve.</button>
								  	<?php }elseif($value->exp_status == 'isPMG'){?>
								  		<button class="btn btn-success btn-sm" class="button" disabled>Imprest is Approved.</button>
								  	<?php }else{ ?>
								  		<button class="btn btn-danger btn-sm" class="button" disabled>Imprest is Rejected.</button>
								  	<?php } ?>
								  </td>
								  <td style="text-align: right;">
								  	<?php if($this->session->userdata('user_type') == 'ACCOUNTANT' || $this->session->userdata('user_type') == 'CRM' || $this->session->userdata('user_type') == 'BOP' || $this->session->userdata('user_type') == 'PMG' || $this->session->userdata('user_type') == 'ADMIN'){?>

								  		<?php if($this->session->userdata('user_emid') == $value->em_id ){?>
								  			<?php if($value->exp_status == 'isNON'){?>
								  				<a href="<?php echo base_url()?>imprest/edit_expenditure?I=<?php echo base64_encode($value->imp_id) ?>" class="btn btn-warning btn-sm">Edit</a>
								  			<button type="button" class="btn btn-danger btn-sm myBtn" >Cancel</button>
								  			<input type="hidden" id="leaveId" value="<?php echo $value->imp_id; ?>">
								  			<a href="<?php echo base_url()?>imprest/verify_expenditure?I=<?php echo base64_encode($value->imp_id) ?>" class="btn btn-info btn-sm">Verify</a>
								  			<?php }elseif($value->exp_status == 'isPMG'){?>
								  				<a href="<?php echo base_url()?>imprest/imprest_subsistence_form?I=<?php echo base64_encode($value->imp_id) ?>" class="btn btn-info btn-sm">Go To Imprest Form</a>
								  			<?php }elseif($value->exp_status == 'isREJ'){?>
								  				<a href="<?php echo base_url()?>imprest/edit_expenditure?I=<?php echo base64_encode($value->imp_id) ?>" class="btn btn-warning btn-sm">Edit</a>
								  			<button type="button" class="btn btn-danger btn-sm myBtn" >Cancel</button>
								  			<input type="hidden" id="leaveId" value="<?php echo $value->imp_id; ?>">
								  		...
								  		<?php }else{ ?>
                                            <?php if($value->exp_status == 'isPMG' || $value->exp_status == 'isPAID'){?>
								  	...
								  	<?php }else{ ?>
								  			...
								  			<?php if($value->exp_status == 'isNON' || $value->exp_status == 'isGMBOP' || $value->exp_status == 'isACC' || $value->exp_status == 'isGMCRM' || $value->exp_status == 'isREJ'){?>
								  				<a href="<?php echo base_url()?>imprest/edit_expenditure?I=<?php echo base64_encode($value->imp_id) ?>" class="btn btn-warning btn-sm">Edit</a>
								  			<button type="button" class="btn btn-danger btn-sm myBtn" >Cancel</button>
								  			<input type="hidden" id="leaveId" value="<?php echo $value->imp_id; ?>">
								  			...
								  			<?php }elseif($value->exp_status == 'isPMG'){?>
								  				<a href="<?php echo base_url()?>imprest/imprest_subsistence_form?I=<?php echo base64_encode($value->imp_id) ?>" class="btn btn-info btn-sm">Go To Imprest Form</a>
								  			<?php }elseif($value->exp_status == 'isPAID'){?>
                                                <img src="<?php echo base_url()?>assets/images/success.png">
								  			<?php }else{?>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <form role="form" action="cancel_imprest" method="post">
        ...
        <div style="float: right;padding-right: 30px;padding-bottom: 10px;">
            <input type="hidden" name="imp_id" id="idleave">
