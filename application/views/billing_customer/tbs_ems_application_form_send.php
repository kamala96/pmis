...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        ...
        <h3 class="text-themecolor"><i class="fa fa-clone" style="color:#1976d2"> </i> SPECIAL TARIFF </h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                <li class="breadcrumb-item active"> SPECIAL TARIFF </li>
            </ol>
        </div>
    </div>
    <!-- Container fluid  -->
    ...
    <div class="container-fluid">
        <div class="row m-b-10">
                ...
                <button type="button" class="btn btn-primary"><i class="fa fa-bars"></i><a href="<?php echo base_url(); ?>Bill_Customer_tariff/dashboard" class="text-white"><i class="" aria-hidden="true"></i>   Special Customer Tariff Dashboard </a></button>
            ...
            <div class="col-12">
                <div class="card card-outline-info">
                    <div class="card-header">
                        <h4 class="m-b-0 text-white"> SPECIAL TARIFF APPLICATION FORM                      
                        </h4>
                    </div>
                    <div class="card-body">
                ...
                <?php }?>
                               ...
                               <?php if($this->session->userdata('user_branch')=="Ngara"){?>
                                <div class="form-group col-md-3">
                                    <label> Choose Tariff Branch</label>
                                    <select  name="uniquebranch" value="" class="form-control custom-select" required id="uniquebranch" onChange = "getNgara();">
                                            <option value=""> Choose Tariff Branch </option>
                                            <option value="Rusumo">Rusumo</option>
                                            <option value="Kabanga">Kabanga</option>
                                        </select>
                                <span id="ngara_error" style="color: red;"></span>
                                </div>
                              <?php } ?>
                            ...
                            <input type="number" readonly name="weight" class="form-control catweight" id="weight" onkeyup="getPriceFrom()" required="required">
...
if (branch=="Tarakea(holoholo)" || branch=="Namanga" || branch=="Mtukula" || branch=="Tunduma" || branch=="Kyela" || branch=="Tanga-HPO" || branch=="Kyaka" || branch=="Rusumo" || branch=="Kabanga" || branch=="Bukoba" || branch=="Kagera HPO" || branch=="Karagwe"){

    $.ajax({
                 type : "POST",
                 url  : "<?php echo base_url('Bill_Customer_tariff/Ems_price_vat')?>",
                 //dataType : "JSON",
                 data : {weight:weight,branch:branch},
    ...
    $.ajax({
                 type : "POST",
                 url  : "<?php echo base_url('Bill_Customer_tariff/Ems_price_vat_other')?>",
                 //dataType : "JSON",
...
var uniquebranch = $('#uniquebranch').val();
if ($('#uniquebranch').val() == "") {
  $('#ngara_error').html('Please Select Tariff Branch');
  $('#Barcode').attr('disabled','disabled');
 } else {
$('.cbranch').val(uniquebranch);
$('#ngara_error').html('');
...
if (branch=="Tarakea(holoholo)" || branch=="Namanga" || branch=="Mtukula" || branch=="Tunduma" || branch=="Kyela" || branch=="Tanga-HPO" || branch=="Kyaka" || branch=="Rusumo" || branch=="Kabanga" || branch=="Bukoba"){
                ...
                if (address == 'physical') {
                    ...
                    //  }
                        ...
                        var cbranch  = $('#cbranch').val();
                            ...
                            $.ajax({
                             type : "POST",
                             url  : "<?php echo base_url('Ems_Domestic/tbs_tanzania_document_parcel_bill_bulk_save')?>",
                             //dataType : "JSON",
                             data : {weight:weight,emstype:emstype,emsCat:emsCat,Barcode:Barcode,
                                    ...
                                    r_mobile:r_mobile,r_mobilev:r_mobilev,region_to:region_to,district:district,serial:serial,cbranch:cbranch},
                             success: function(data){
                                 $('#div6').show();
                                $('.list').html(data);
                                $('.price').html('');
                     ...
                     //$('.disable').attr("disabled", true);
                        ...
                        var cbranch  = $('#cbranch').val();
                            ...
                            $.ajax({
                             ...
                             data : {weight:weight,emstype:emstype,emsCat:emsCat,Barcode:Barcode,
                                    sender:sender,receiverselect:receiverselect,r_fname:r_fname,r_address:r_address,r_email:r_email,
                                    r_mobile:r_mobile,r_mobilev:r_mobilev,region_to:region_to,district:district,askfor:askfor,serial:serial,cbranch:cbranch},
                             success: function(data){
                                 $('#div6').show();
                                $('.list').html(data);
                                $('.price').html('');
