	...
	$controlno    = $this->input->post('bill');
        ...
        if (!empty(@$check) || !empty(@$check2) || !empty(@$check3) || !empty(@$check4) || !empty(@$check6) || !empty(@$check7) ){
 				  ...
 				  $serial=$check->serial;
                    ...
                    $serviceId = $check->PaymentFor;

                  @$repost = $this->getBillGepgBillId($serial, $paidamount,$region,$district,$mobile,$renter,$serviceId);


        			$amount = @$repost->amount;
			$controlno =@$repost->controlno;// $update1['controlno'];
			$receiptno = @$repost->receiptno; //$update1['receiptno'];
			$channel   = @$repost->channel; //$update1['channel'];
			$date = @$repost->date; //$update1['date'];
