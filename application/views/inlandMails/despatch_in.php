      ...
      <h3 class="text-themecolor"><i class="fa fa-braille" style="color:#1976d2"></i>&nbsp 
         <?php 
            ...
            $basicinfo = $this->employee_model->GetBasic($id); 
                            ...
                            //        } ?>
         <?php echo $this->session->userdata('service_type'); ?> Dashboard  Back Office
      </h3>
   </div>
   ...
   <div class="row">
      <div class="col-md-12">
         ...
         <?php if ($current_section){ ?>
            ...
            <a href="<?php echo base_url(); ?>Services/<?php echo $current_controller; ?>?Ask_for=<?php echo $this->session->userdata('service_type'); ?>" class="btn btn-info" style="padding-right: 10px;">Return to Dashboard</a>
            ...
            <div class="card-body">
               <div class="row">
                  <div class="col-md-12 table table-responsive">
                     ...
                     <?php }else{?>
                     <?php } ?>
                     <form action="despatch_in?Ask_for=<?php echo $this->session->userdata('service_type'); ?>" method="POST">
                        <table class="table table-bordered">
                           <tr>
                              <th><input type="text" name="date" class="form-control mydatetimepickerFull" placeholder="Choose Date" style="height: 42px;"></th>
                              <th><input type="text" name="month" class="form-control mydatetimepicker" placeholder="Choose Month" style="height: 42px;"></th>
                              <th>
                                 <select class="form-control custom-select" name="status"  style="height: 42px;">
                        ...
                        <table id="example4" class="display nowrap table table-hover  table-bordered" cellspacing="0" width="100%" style="text-transform: capitalize;">
                           <thead>
                              <tr style="background-color: oblique;">
                                 ...
                                 <th>
                                    <div class="form-check" style="text-align: center;" id="showCheck">
                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td><a href="bags_item_list_by_despatchno?despno=<?php echo base64_encode($value->desp_no ); ?>&& Ask_for=<?php echo $this->session->userdata('service_type') ?> && type=Despin" class="btn btn-info"><?php echo $value->desp_no ?></a></td>
                                 <td><?php echo $value->origin_region ?></td>
                                 <td><?php echo $value->branch_origin?></td>
                                 <td><?php echo $value->region_to ?></td>
                                 <td><?php  echo $value->item_number ?></td>
                                 <td style="text-align: center;">
                                    <?php if($value->despatch_status == "Sent"){ ?>
                                    <button class="btn btn-info" disabled="disabled">Despatched</button>
                                    <?php }else{?>
                                 ...
                                 <td>
                                    ...
                                    <div class='form-check' style="text-align: center;">
                                       <input data-despatchid='<?php echo $value->desp_id; ?>' type='checkbox' name='I[]' class='form-check-input checkSingles' 
                                       id='remember-me<?php echo $value->desp_no; ?>' value='<?php echo $value->desp_no; ?>' style="zoom:2px;">
                                       <label class='form-check-label' 
                                       for='remember-me<?php echo $value->desp_no; ?>'></label>
                                    </div>
                                 </td>
                              </tr>
                              <?php } ?>
                           ...
                           <tfoot>
                              <tr>
                                 ...
                                 <td colspan="" style="text-align: center;">
                                    <!-- <button class="btn btn-info" name="receive" value="receive">Receive Item</button> -->
                                    <!-- <button class="btn btn-info" name="receive" value="receive">Receive Item</button> -->
                                    <div class='btn btn-info receiveBags' onclick='receiveBags(this)'>Receive Despatch</div>
...
<script type="text/javascript">
   ...
   function receiveBags(obj){
      ...
      $(".checkSingles").each(function(){
         ...
         if($(this).prop("checked") == true){
            //checked despatch
            var despatch = $(this).val()
            var despatchid = $(this).attr('data-despatchid');//.val();
            
            $.ajax({
               type: "POST",
               url: "<?php echo base_url();?>unregistered/receiveBags",
               data:'despno='+ despatch+'&despatchid='+despatchid,
   ...
   function receiveBagItems(obj){
     ...
     $('.statusText').html('<h3><strong>Please wait (Tafadhali subiri) .......</strong></h3>');
       ...
       var bagid = $(obj).attr('data-bagid');

       $.ajax({
          type: "POST",
          url: "<?php echo base_url();?>unregistered/receiveBagItems",
          data:'despatchno='+despatch+'&bagno='+bagno+'&bagid='+bagid,
          success: function(response) {
            //console.log(response)
            $('#example4_wrapper').hide();
              $('.despatch1').hide();
   ...
   function formSubmit(obj){
    ...
    $.ajax({
           type: "POST",
           url: "<?php echo base_url();?>unregistered/receive_action",
           data: 'selected='+selectedData+'&despatchno='+despatchno+'&barcode='+barcode, 
           success: function(response){
            //console.log(response)
