...
<table style="width: 80%;">
    <tr style="">
            ...
            ?></td>
             ...
             <?php $assAfricaamount = 0;?>
            <?php foreach(@$assAfrica as $value): ?>
             <tr>
            ...
            <td style="text-align: right;"><?php 
                if ($value->ded_amount == 0) {
                   ...
                   echo number_format($value->ded_amount,2);
                   $assAfricaamount = $assAfricaamount + $value->ded_amount ;
                }
       ...
       <!--  <?php foreach($NonTaxDeduction1 as $value): ?>
        ...
        <?php  foreach($LoanDeduction as $value): ?>
            ...
            <td style="text-align: right;"><?php //if ($value->installment_Amount < 0) {
        ...
        <tr style="">
            ...
            <td style="text-align: right;"> 
                ----------------<br>
                <?php
                $TotaDeduction = ($salary_info->others_deduction_total + $salary_info->percentage_deduction_total + $salary_info->nonpercentage_deduction_total + $salary_info->paye + $salary_info->pension_fund + $assAfricaamount);
        ...
        <tr style="">
            ...
            <td><b><?php
            $namemonth = $salary_info->month;
            $nmonth = date('m',strtotime($namemonth));
            
             echo $namemonth.'/'.$salary_info->year  ?></b></td>
        ...
        <?php foreach($NonTaxDeduction1 as $value): ?>
            ...
            if ($value->ded_name == "TEWUTA" || $value->ded_name == "COTWU(T)") {
                ...
                $id = $value->em_id;
                //$total = $this->payroll_model->getSumNonTaxDeductiontotal($ded_name,$id);
                 $totalpreviousyear = $this->payroll_model->getSumNonTaxDeductiontotalpreviousyear($ded_name,$id,$month,$year);
                
                 $totalcurrentyear = $this->payroll_model->getSumNonTaxDeductiontotalcurrentyear($ded_name,$id,$month,$year);
                // $totalcurrentyear = $this->payroll_model->getSumNonTaxDeductiontotalcurrentyear('KK SAVINGS','Ndy1849','DECEMBER','2021');
                $total =intval($totalpreviousyear->ded_amount) + intval($totalcurrentyear->ded_amount);
                 //$total =$totalcurrentyear->ded_amount;
                // $salary_id = $value->salary_id;
            //$total =$this->payroll_model->getSumNonTaxDeduction($ded_name,$month,$year,$salary_id);
                 if (empty($totalcurrentyear->ded_amount) && empty($totalpreviousyear->ded_amount) ) {
                    echo number_format(0,2);
                } else {
        ...
        <?php endforeach; ?>
            ...
            <?php if($pensionFund > 0){ ?>

            <tr>
            <td>PSSSF</td>
            <td></td>
            <td style="text-align: right;"><?php echo number_format(@$pensionFund,2);?></td>
            </tr>
        <?php }else{?>
