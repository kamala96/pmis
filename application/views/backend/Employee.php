   ...
   public function GetDistrict(){
    ...
    public function Create_Salary()
    {
        if($this->session->userdata('user_login_access') != False) { 
            ...
            $this->load->view('backend/create_salary',$data);
        }
    ...
    public function Search()
    {
        if($this->session->userdata('user_login_access') != False) {
            ...
            $em_gender = $this->input->get('em_gender');
            $region = $this->input->get('region');
            $branch = $this->input->get('branch');

            
          if (!empty($em_code)) {
               $employee = $this->employee_model->emselectSearch($em_code);
          }elseif (!empty($em_gender)) {
               $employee = $this->employee_model->emselectGender($em_gender);
          }
          elseif (!empty($region)) {
               $employee = $this->employee_model->emselectRegion($region);
          }
          elseif (!empty($em_gender) && !empty($region) && !empty($branch)) {
               $employee = $this->employee_model->emselectMultiple($em_gender,$region,$branch);
          } elseif(!empty($region) && !empty($branch)) {
               $employee = $this->employee_model->emselectMultiple2($region,$branch);
          }elseif (!empty($em_gender) && !empty($region)) {
               $employee = $this->employee_model->emselectMultiple3($em_gender,$region);
          }
