<?php
	...
	class Employee_model extends CI_Model{
		...
		$db2 = $this->load->database('otherdb', TRUE);
		$sql1    = "SELECT `branch_bill_credit_customer`.*,`bill_credit_customer`.* FROM `bill_credit_customer`  
          INNER JOIN `branch_bill_credit_customer` ON `branch_bill_credit_customer`.`acc_no` = `bill_credit_customer`.`acc_no` 
           WHERE `bill_credit_customer`.`acc_no` = '$acc' AND `branch_bill_credit_customer`.`customer_region` = '$reg' GROUP BY `branch_bill_credit_customer`.`customer_branch`";
...
foreach ($query->result() as $row) {
	if (empty($result1)) {
		$output .= "&nbsp;<label>".$row->branch_name.'</label>  &nbsp;'."<input type='checkbox' id='vehicle1' name='branch[]' style='text-transform:uppercase;' value='".$row->branch_name."' style='height:45px;width:45px;';
			        padding: 10px;' >".'      ';
	} else {
		foreach ($result1 as $value) {
				if ($value->customer_branch == $row->branch_name) {
					$checked = 'checked';
			     break;
				} else{
					$checked = ' ';
...
$output .= "&nbsp;<label>".$row->branch_name.'</label>  &nbsp;'."<input type='checkbox' id='vehicle1' name='branch[]' value='".$row->branch_name."' style='height:45px;width:45px;';
	...
	public function emselectAgent(){
		  ...
		  }elseif ($this->session->userdata('user_type') == 'HOD')
		  {
			  ...
			  $sql = "SELECT * FROM `employee` WHERE `status`='ACTIVE' AND `em_region` = '$region' ORDER BY `em_code` ASC";
		  }
		  else{
		  ...
		  if ($this->session->userdata('user_type')=='RM') {
			  $sql = "SELECT * FROM `employee`
			  WHERE `status`='INACTIVE' AND `em_region` = '$region'";
		  }elseif ($this->session->userdata('user_type')=='HOD') {
			  $sql = "SELECT * FROM `employee`
		  ...
		  if ($this->session->userdata('user_type')=='RM') {
			...
			public function GetSundryValue($sid){
			  $month = date('m');
			  $year  = date('Y');
			  $sql = "SELECT * FROM `non_tax_addition` WHERE `salary_id` = '1427' 
...
OR ((MONTH(`end_month`) <= '$month' AND YEAR(`end_month`) > '$year')))";
			...
			public function GetCommulativePension($sid,$id){
		...
		public function Update_Family($id,$data){
		  ...
		  public function Delete_others_deduction_permonth($id){
			  $this->db->delete('others_deduction_permonth',array('others_id'=> $id));
		  }
